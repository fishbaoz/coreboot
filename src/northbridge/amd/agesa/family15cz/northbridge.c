...
static void northbridge_fill_ssdt_generator(void)
{
	...
	printk(BIOS_DEBUG, "northbridge_fill_ssdt_generator\n");
	msr = rdmsr(TOP_MEM2);
	/*
...
static const struct pci_driver family15_northbridge __pci_driver = {
	...
	.device = 0x1570, //PCI_DEVICE_ID_AMD_15H_MODEL_003F_NB_HT,
};
static const struct pci_driver family15_a1_northbridge __pci_driver = {
	.ops	= &northbridge_operations,
	.vendor = PCI_VENDOR_ID_AMD,
	.device = 0x1576, //PCI_DEVICE_ID_AMD_15H_MODEL_003F_NB_HT,
};
...
struct chip_operations northbridge_amd_agesa_family15cz_ops = {
...
{
#if CONFIG_GFXUMA
	uint64_t topmem = (uint64_t) bsp_topmem();
	uint64_t topmem2 = (uint64_t) bsp_topmem2();
	uint32_t sys_mem;
	...
	printk(BIOS_DEBUG, "sys_mem=%llx\n", sys_mem);
	printk(BIOS_DEBUG, "topmem=%llx, topmem2=%llx\n", topmem, topmem2);
	if (topmem2 > 0x180000000ul) {
		uma_memory_size = 1024 << ONE_MB_SHIFT;
	} else if ((bsp_topmem2()>>32) || (sys_mem >= 2048 << ONE_MB_SHIFT)) {
...
struct chip_operations northbridge_amd_agesa_family15cz_root_complex_ops = {
