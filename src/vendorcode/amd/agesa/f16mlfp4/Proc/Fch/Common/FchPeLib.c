  ...
  )
{
  ...
  if (pGpioTbl != NULL) {
    while (pGpioTbl->GpioPin != 0xFF) {
      ...
      ddtempVar = ACPI_MMIO_BASE | 0x1500 | ((UINT32)(pGpioTbl->GpioPin) << 2);
      ddtempVar1 = (pGpioTbl->CfgByte) << 16;
      LibAmdMemWrite (AccessWidth32, (UINT64) ddtempVar, &ddtempVar1, StdHeader);
      pGpioTbl++;
    }
  }
}
