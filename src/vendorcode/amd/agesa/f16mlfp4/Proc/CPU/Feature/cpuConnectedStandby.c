  ...
  )
{
  ...
  if (FamilyServices != NULL) {
    if ((EntryPoint & CPU_FEAT_AFTER_PM_INIT) != 0) {
    ...
    } else if (((EntryPoint & CPU_FEAT_S3_SAVE_END) || (EntryPoint & CPU_FEAT_INIT_RTB_END)) != 0) {
      // Check if CS restoration data has been successfully published
      LocateHeap.BufferHandle = AMD_CS_RESTORATION_HANDLE;
      if (HeapLocateBuffer (&LocateHeap, StdHeader) != AGESA_SUCCESS) {
        ...
        if ((CalledStatus == AGESA_SUCCESS) && (CsCalloutParams.CsRestoreTable != NULL) && (CsCalloutParams.CsRestoreTableSize != 0)) {
          ...
          // Set flag if CS restoration data has been successfully published
          if (CalledStatus == AGESA_SUCCESS) {
            // Allocate a 1-byte buffer as flag without content
            AllocHeapParams.RequestedBufferSize = 1;
            AllocHeapParams.BufferHandle = AMD_CS_RESTORATION_HANDLE;
...
{
  ConnectedStandby,
  (CPU_FEAT_AFTER_PM_INIT | CPU_FEAT_S3_SAVE_END | CPU_FEAT_INIT_RTB_END),
  IsConnectedStandbyFeatureEnabled,
  InitializeConnectedStandbyFeature
};
