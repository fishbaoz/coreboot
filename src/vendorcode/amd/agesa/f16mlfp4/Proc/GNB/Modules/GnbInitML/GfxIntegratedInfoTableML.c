 ...
 * @e project:     AGESA
 * @e sub-project: GNB
 * @e \$Revision: 282646 $   @e \$Date: 2014-01-14 14:44:02 -0600 (Tue, 14 Jan 2014) $
 *
 */
/*
*****************************************************************************
  ...
  )
{
  ...
  BiosTable = (SMU7_BIOSTABLE *) GnbLocateHeapBuffer (AMD_GNB_SMU_TABLE_HANDLE, GnbLibGetHeader (Gfx));
  ASSERT (BiosTable != NULL);
  if (D18F3x64.Field.HtcEn == 1) {
    if (BiosTable->TemperatureLimit != 0) {
      IntegratedInfoTable->ucHtcTmpLmt = (UCHAR) (BiosTable->TemperatureLimit / 256);
    } else {
