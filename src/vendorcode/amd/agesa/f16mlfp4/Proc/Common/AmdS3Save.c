...
AGESA_STATUS
AmdS3SaveInitializerCommon (
  IN       AMD_CONFIG_PARAMS *StdHeader,
  IN OUT   AMD_S3_PARAMS     *S3Params
  );

AGESA_STATUS
AmdS3SaveDestructorCommon (
  IN       AMD_CONFIG_PARAMS *StdHeader,
  IN OUT   AMD_S3_PARAMS     *S3Params
  );
 ...
 *
 * Note: This entry point is deprecated in favor of AmdInitRtb.
 *       This entry point will be removed at a future date.
 *
  ...
  )
{
  ...
  IDS_SKIP_HOOK (IDS_BEFORE_S3_SAVE, AmdS3SaveParams, &(AmdS3SaveParams->StdHeader)) {
    AgesaStatus = AmdS3SaveCommon (AmdS3SaveParams);
...
/*---------------------------------------------------------------------------------------*/
/**
 * Wrapper function for AMD_INIT_RTB entry point to the AMD_S3_SAVE function.
 *
 * This entry point is responsible for saving silicon component registers to the
 * SMM save area in preparation of entering system suspend-to-RAM mode.
 *
 * @param[in,out] AmdRtbParams      Required input parameters for the AMD_S3_SAVE
 *                                  entry point.
 *
 * @return        Aggregated status across all internal AMD S3 save calls invoked.
 *
  ...
  )
{
  AGESA_STATUS ReturnStatus;
  AGESA_STATUS AgesaStatus;
  AMD_S3SAVE_PARAMS       AmdS3SaveParams;

  AGESA_TESTPOINT (TpIfAmdS3SaveEntry, &AmdRtbParams->StdHeader);

  ASSERT (AmdRtbParams != NULL);

  ReturnStatus = AGESA_SUCCESS;

  // Pass input parameters per AMD_S3SAVE_PARAMS and AMD_RTB_PARAMS declaration
  AmdS3SaveParams.StdHeader = AmdRtbParams->StdHeader;
  AmdS3SaveParams.PlatformConfig = AmdRtbParams->PlatformConfig;
  AmdS3SaveParams.S3DataBlock = AmdRtbParams->S3DataBlock;
  AmdS3SaveParams.FchInterface = AmdRtbParams->FchInterface;
  ...
  IDS_SKIP_HOOK (IDS_BEFORE_S3_SAVE, &AmdS3SaveParams, &AmdS3SaveParams.StdHeader) {
    ...
    // Retrieve output parameters per AMD_S3SAVE_PARAMS and AMD_RTB_PARAMS declaration
    AmdRtbParams->StdHeader = AmdS3SaveParams.StdHeader;
    AmdRtbParams->S3DataBlock = AmdS3SaveParams.S3DataBlock;
  }

  IDS_OPTION_HOOK (IDS_AFTER_S3_SAVE, &AmdS3SaveParams, &AmdS3SaveParams.StdHeader);
  AGESA_TESTPOINT (TpIfAmdS3SaveExit, &AmdRtbParams->StdHeader);
  return  ReturnStatus;
}
  ...
  )
{
  ...
  AmdS3SaveInitializerCommon (&S3SaveParams->StdHeader, &S3SaveParams->S3DataBlock);
...
/*---------------------------------------------------------------------------------------*/
/**
 * Common constructor for the AMD_INIT_RTB and AMD_S3_SAVE function.
 *
 * This routine is responsible for setting default values for the
 * input parameters needed by the AMD_INIT_RTB and AMD_S3_SAVE entry point.
 *
 * @param[in]     StdHeader      The standard header.
 * @param[in,out] S3SaveParams   Required input parameters for the AMD_INIT_RTB and AMD_S3_SAVE
 *                               entry point.
 *
 * @retval        AGESA_SUCCESS  Always Succeeds.
 *
...
AmdS3SaveInitializerCommon (
  IN       AMD_CONFIG_PARAMS *StdHeader,
  IN OUT   AMD_S3_PARAMS     *S3SaveParams
  )
{
  AmdS3ParamsInitializer (S3SaveParams);
  return AGESA_SUCCESS;
}

/*---------------------------------------------------------------------------------------*/
/**
 * Common destructor for the AMD_INIT_RTB and AMD_S3_SAVE function.
 *
  ...
  )
{
  ...
  RetVal = AmdS3SaveDestructorCommon (StdHeader, &S3SaveParams->S3DataBlock);
  if (RetVal > ReturnStatus) {
    ReturnStatus = RetVal;
  }
