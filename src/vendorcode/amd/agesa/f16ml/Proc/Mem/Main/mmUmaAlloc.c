  ...
  )
{
  ...
  case UMA_NONE:
    UmaSize = 0;
    printk(BIOS_DEBUG, "%s, %d:UmaSize=%x\n", __func__, __LINE__, UmaSize);
    break;
  case UMA_SPECIFIED:
  ...
  if (UmaSize != 0) {
    ...
    printk(BIOS_DEBUG, "%s, %d: RefPtr->UmaBase=%x\n", __func__, __LINE__, RefPtr->UmaBase);
    ...
    if (RefPtr->UmaBase >= _4GB_RJ16) {
      ...
      printk(BIOS_DEBUG, "%s, %d: RefPtr->UmaBase=%x, RefPtr->UmaSize=%x\n", __func__, __LINE__, RefPtr->UmaBase, RefPtr->UmaSize);
