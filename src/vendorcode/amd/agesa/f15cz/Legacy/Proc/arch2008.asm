...
;     memory.  The host environment must fill the S3_DATA_BLOCK NvStorage
...
AmdInitResumeWrapper PROC NEAR PUBLIC
    ...
    mov   edi, esi
    add   edi, (SIZEOF AMD_CONFIG_PARAMS + SIZEOF S3_DATA_BLOCK)
    call  oemPlatformConfigInit
...
;
;   Entry:
;     On Entry to "AmdInitRtb" AGESA will display AGESA_TESTPOINT - CEh
;     DS - 0000 with 4 gigabyte access
;     ES - 0000 with 4 gigabyte access
;
;   Exit:
;     On Entry to "AmdInitRtb" AGESA will display AGESA_TESTPOINT - CFh
;     None
;
;   Modified:
;     None
...
;     pointers and sizes within the S3_DATA_BLOCK structure will be updated
...
;     the stack environment prior to making the call to this procedure.
;
AmdInitRtbWrapper PROC NEAR PUBLIC
    local localCfgBlock:AMD_INTERFACE_PARAMS

    pushad

    ; Prepare for the call to create and initialize the input parameters for AmdInitRtb
    xor   eax, eax
    mov   ax, ss
    shl   eax, 4
    lea   esi, localCfgBlock
    ...
    mov   (AMD_INTERFACE_PARAMS ptr [esi]).AgesaFunctionName, AMD_INIT_RTB
    mov   (AMD_INTERFACE_PARAMS ptr [esi]).AllocationMethod, PostMemDram
    mov   (AMD_INTERFACE_PARAMS ptr [esi]).NewStructSize, 0
    push  esi
    mov   dx, SEG AmdCalloutRouterPost
    ...
    ; Call in to the AmdInitRtb entry point
    push  edx
    call  AmdBridge32
    pop   edx
    ...
    .if (eax != AGESA_SUCCESS)
        mov   al, (AMD_RTB_PARAMS ptr [esi]).StdHeader.HeapStatus
        mov   ebx, AGESA_B2_ADDRESS
        call  AmdProcessAgesaErrors
    .endif

    mov   ecx, (AMD_RTB_PARAMS ptr [esi]).S3DataBlock.NvStorageSize
    .if (ecx != 0)
        mov   ebx, (AMD_RTB_PARAMS ptr [esi]).S3DataBlock.NvStorage
        call  mySaveNonVolatileS3Context
    .endif

    mov   ecx, (AMD_RTB_PARAMS ptr [esi]).S3DataBlock.VolatileStorageSize
    .if (ecx != 0)
        mov   ebx, (AMD_RTB_PARAMS ptr [esi]).S3DataBlock.VolatileStorage
        call  mySaveVolatileS3Context
    .endif

    ; Allow AGESA to free the space used by AmdInitRtb
    pop   esi
    mov   (AMD_INTERFACE_PARAMS ptr [esi]).StdHeader.Func, AMD_RELEASE_STRUCT
    call  AmdBridge32
...
;     processor registers.
;
;     The host environment must fill the S3_DATA_BLOCK NvStorage and
...
;     AmdInitMid, AmdInitLate, AmdInitRtb, and
;     AmdS3LateRestore.
;
;   Dependencies:
;     None
