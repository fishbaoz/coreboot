...
STATIC BOOLEAN      AllAps;
STATIC UINT32       NumAps;
/*---------------------------------------------------------------------------------------*/
/**
  ...
  )
{
  ...
  AP_EXE_PARAMS        *ApExeParamsPtr;
  //
  // Invoke AmdAmdCpuLateInit
  //
  AmdInterfaceParams.AllocationMethod = ByHost;
  AmdInterfaceParams.AgesaFunctionName = AMD_LATE_RUN_AP_TASK;
  ApExeParamsPtr = &ApExeParams;
  AmdInterfaceParams.NewStructPtr = ApExeParamsPtr;
  AmdInterfaceParams.NewStructSize = sizeof (AP_EXE_PARAMS);

  AgesaStatus = AmdCreateStruct (&AmdInterfaceParams);
  ApExeParamsPtr = (AP_EXE_PARAMS *)AmdInterfaceParams.NewStructPtr;

  ApExeParamsPtr->RelatedBlockLength = RelatedBlockLength;
  ApExeParamsPtr->RelatedDataBlock = RelatedDataBlock;
  ApExeParamsPtr->FunctionNumber = FunctionNumber;

  AgesaStatus = AmdLateRunApTask (ApExeParamsPtr);

  AmdReleaseStruct (&AmdInterfaceParams);
  if (AllAps) {
    InterlockedDecrement ((UINT32 *)&NumAps);
  }
 ...
 *    Call the host environment interface to provide a user hook opportunity.
 *
 *  @param[in]      FcnData          (unused)
 *  @param[in]      LaunchApParams
 *
 * @return   The AGESA Status returned from the callout.
 *
 **/
  ...
  )
{
  ...
  FunctionNumber = LaunchApParams->FunctionNumber;
  AllAps = LaunchApParams->AllAPs;
  GetActiveCoresInCurrentSocket (&NumAps, &LaunchApParams->StdHeader);
  NumAps--;
 ...
 *
 * Description
 *    Places an AP in the idle state.
 *
 *  @param[in]      ApicIdOfCore
 *  @param[in]      StdHeader
 *
  ...
  )
{
  ...
  Status = MpServices->EnableDisableAP (
           MpServices,
           CpuData_Index,
           TRUE,
           NULL
 ...
 *
 * Description
 *    Wait for all APs to complete a task.
 *
 * @param[in]      FcnData     (unused)
 * @param[in]      StdHeader
 *
