...
#include "FchIntDev.h"
#include "Filecode.h"
#define FILECODE UEFI_FCH_KERN_DXE_FCHACPICS_FCHACPICSCONTROL_FILECODE
  ...
  )
{
  ...
  while ((UINTN)CurrentAdrAml < ((UINTN)AmlStart + (UINTN)AmlSize)) {
    ...
    FchAcpiPkgLength ((UINT8*) ((UINTN)CurrentPci0Aml + 1), &ByteCount, &Pci0Length);
  ...
  )
{
  ...
  LibFchIndirectIoRead (EfiPciWidthUint8, FCH_IOMAP_REGC00, FCH_IRQ_GPIO | FCH_IRQ_IOAPIC, &GpioIrq);
