  ...
  )
{
  ...
  // Get CPU information
  //
  CalledStatus = GetType4Type7Info (StdHeader, DmiBufferPtr);
 ...
 *
 *  Description:
 *    This routine should be run on BSC only. It creates DMI type 4 and type 7 tables.
 *
 *  Parameters:
 *    @param[in]      StdHeader         Config handle for library and services
 *    @param[in]      DmiBufferPtr      DMI buffer pointer
  ...
  )
{
  ...
  ProcData->DmiGetCpuInfo (&CpuInfo, StdHeader);

  // ------------------------------
  // T Y P E 4
  // ------------------------------

  IdentifyCore (StdHeader, &SocketNum, &IgnoredModule, &IgnoredCore, &IgnoredSts);
  ...
  ProcData->DmiGetT4ProcFamily (&DmiBufferPtr->T4[SocketNum].T4ProcFamily, ProcData, &CpuInfo, StdHeader);
  ...
  DmiBufferPtr->T4[SocketNum].T4Voltage = ProcData->DmiGetVoltage (StdHeader);

  // Type4 Offset 0x12, External Clock
  DmiBufferPtr->T4[SocketNum].T4ExternalClock = ProcData->DmiGetExtClock (StdHeader);

  // Type4 Offset 0x14, Max Speed
  DmiBufferPtr->T4[SocketNum].T4MaxSpeed = ProcData->DmiGetMaxSpeed (StdHeader);
  ...
  for (Index = 0; Index <= 5; Index++) {
    LibAmdMsrRead ((MSR_CPUID_NAME_STRING0 + Index), &MsrData, StdHeader);
  ...
  LibAmdMemCopy (DmiBufferPtr->T4[SocketNum].T4ProcManufacturer, str_ProcManufacturer, sizeof (str_ProcManufacturer), StdHeader);
