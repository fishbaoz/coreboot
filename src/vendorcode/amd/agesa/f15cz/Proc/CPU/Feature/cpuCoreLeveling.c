 ...
 *
 * Contains code to Level the number of core
 *
  ...
  )
{
  ...
  // CmpCap
  PciAddress.AddressValue = MAKE_SBDFO (0, 0, 24, FUNC_5, NORTH_BRIDGE_CAPABILITIES_2_REG);
  LibAmdPciRead (AccessWidth32, PciAddress, &NbCap2, StdHeader);
  // CoreDis
  PciAddress.AddressValue = MAKE_SBDFO (0, 0, 24, FUNC_3, DOWNCORE_CTRL);
  LibAmdPciRead (AccessWidth32, PciAddress, &CoreDis, StdHeader);

  TotalEnabledCoresOnNode = (NbCap2 & 0xFF) + 1;
  while (CoreDis != 0) {
    if ((CoreDis & 1) == 1) {
  ...
  GetFeatureServicesOfCurrentCore (&CoreLevelingFamilyServiceTable, (CONST VOID **)&FamilySpecificServices, StdHeader);
