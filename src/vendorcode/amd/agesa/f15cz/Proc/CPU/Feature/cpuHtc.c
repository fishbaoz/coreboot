  ...
  )
{
  ...
  if ((HtcFamilyServices == NULL) || (!HtcFamilyServices->IsHtcSupported (HtcFamilyServices, PlatformConfig, StdHeader))) {
    IsEnabled = FALSE;
  }
  ...
  )
{
  ...
  if (HtcFamilyServices != NULL) {
    AgesaStatus = HtcFamilyServices->EnableHtc (HtcFamilyServices, EntryPoint, PlatformConfig, StdHeader);
  }
