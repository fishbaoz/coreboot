  ...
  )
{
  ...
  if ((PsiFamilyServices == NULL) || (!PsiFamilyServices->IsPsiSupported (PsiFamilyServices, PlatformConfig, StdHeader))) {
    IsEnabled = FALSE;
  }
  return IsEnabled;
}
  ...
  )
{
  ...
  if (PsiFamilyServices != NULL) {
    AgesaStatus = PsiFamilyServices->EnablePsi (PsiFamilyServices, EntryPoint, PlatformConfig, StdHeader);
  }
