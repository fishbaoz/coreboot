  ...
  )
{
  ...
  if (PlatformConfig->CpbMode == CpbModeAuto) {
    GetFeatureServicesOfCurrentCore (&CpbFamilyServiceTable, (CONST VOID **)&FamilyServices, StdHeader);
    if (FamilyServices != NULL) {
      if (FamilyServices->IsCpbSupported (FamilyServices, PlatformConfig, StdHeader)) {
        IsEnabled = TRUE;
      }
    }
  }
  ...
  )
{
  ...
  GetFeatureServicesOfCurrentCore (&CpbFamilyServiceTable, (CONST VOID **)&FamilyServices, StdHeader);
  if (FamilyServices != NULL) {
    CalledStatus = FamilyServices->EnableCpb (FamilyServices, PlatformConfig, EntryPoint, StdHeader);
...
{
  CoreBoost,
  (CPU_FEAT_MID_INIT | CPU_FEAT_MID_LATE_INIT),
  IsCpbFeatureEnabled,
  InitializeCpbFeature
};
