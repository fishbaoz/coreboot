 ...
 * @e project:      AGESA
 * @e sub-project:  CPU
 * @e \$Revision: 313729 $   @e \$Date: 2015-02-26 01:28:54 -0600 (Thu, 26 Feb 2015) $
 *
 */
/*
 ******************************************************************************
  ...
  )
{
  ...
  if (IsBsp (StdHeader)) {
    ...
    GetCpuServicesOfCurrentCore ((CONST CPU_SPECIFIC_SERVICES **) &FamilySpecificServices, StdHeader);
    GetGivenModuleCoreRange (0, 0, &PrimaryCore, &HighCore, StdHeader);

    for (i = (UINT8) PrimaryCore + 1; i <= (UINT8) HighCore; i++) {
      IDS_HDT_CONSOLE (CPU_TRACE, "  Launch core %d\n", i);
      if (FamilySpecificServices->LaunchApCore (FamilySpecificServices, i, StdHeader)) {
        IDS_HDT_CONSOLE (CPU_TRACE, "  Waiting for core %d\n", i);
        ...
        ApUtilWaitForCoreStatus (0, i, &WaitForStatus, StdHeader);
        ApHeapIndex++;
      }
    }
    ...
    OptionApCommConfiguration.ApWriteCtrlByte (CORE_IDLE_HLT, StdHeader);
  } else {
    OptionApCommConfiguration.ApEntry (StdHeader, &CpuEarlyParams);
  }
