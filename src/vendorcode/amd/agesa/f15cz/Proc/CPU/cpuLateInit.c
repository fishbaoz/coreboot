  ...
  )
{
  ...
  PciAddress.AddressValue = MAKE_SBDFO (0, 0, 0x18, FUNC_3, NB_CFG_HIGH_REG);
  ...
  )
{
  ...
  if (ApParams->AllAPs) {
    ...
    CalledStatus = AgesaRunFcnOnAllAps (0, ApParams);
    ...
    for (Core = 1; Core < NumberOfCores; Core++) {
      GetApicId (StdHeader, 0, Core, &ApicId, &IgnoredStatus);
      AGESA_TESTPOINT (TpIfBeforeRunApFromAllAps, StdHeader);
...
/**
 *
 * Idle all Aps after a run all simultaneously call
 *
 * @param[in] StdHeader      Handle to config for library and services
 *
 *
  ...
  )
{
  ...
  AgesaStatus = AgesaWaitForAllApsFinished (0, StdHeader);

  GetActiveCoresInCurrentSocket (&NumberOfCores, StdHeader);
  for (Core = 1; Core < NumberOfCores; Core++) {
    GetApicId (StdHeader, 0, Core, &ApicId, &IgnoredStatus);
    CalledStatus = AgesaIdleAnAp ((UINTN) ApicId, StdHeader);
    if (CalledStatus > AgesaStatus) {
      AgesaStatus = CalledStatus;
    }
  }
...
/**
  ...
  specified by Value and returns the decremented value. The
  decrement operation must be performed using MP safe
  mechanisms. The state of the return value is not guaranteed to
  be MP safe.
...
{
  ASSERT (Value != NULL);
  return InternalDecrement (Value);
}
