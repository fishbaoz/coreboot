 ...
 * @file
 *
 * AMD Family 15h Stoney IO C-state feature support functions.
 *
...
CODE_GROUP (G3_DXE)
RDATA_GROUP (G3_DXE)
#define FILECODE PROC_CPU_FAMILY_0X15_ST_CPUF15STIOCSTATE_FILECODE
...
VOID
STATIC
F15StInitializeIoCstateOnCore (
  IN       VOID *CstateBaseMsr,
  IN       AMD_CONFIG_PARAMS *StdHeader
  );

BOOLEAN
F15StIsCsdObjGenerated (
...
/*---------------------------------------------------------------------------------------*/
/**
 *  Enable IO Cstate on a family 15h Stoney CPU.
  ...
  )
{
  ...
  if ((EntryPoint & CPU_FEAT_AFTER_PM_INIT) != 0) {
    ...
    TaskPtr.FuncAddress.PfApTaskI = F15StInitializeIoCstateOnCore;
...
/*---------------------------------------------------------------------------------------*/
/**
 *  Enable CState on a family 15h Stoney core.
 *
...
CONST IO_CSTATE_FAMILY_SERVICES ROMDATA F15StIoCstateSupport =
{
  0,
