  ...
  )
{
  ...
  // Code Start
  LaunchFlag = FALSE;
  PciAddress.AddressValue = (MAKE_SBDFO (0, 0, 0x18, FUNC_0, CORE_CTRL));
  LibAmdPciRead (AccessWidth32, PciAddress, &LocalPciRegister, StdHeader);

  if ((LocalPciRegister & (1 << CoreNum)) == 0) {
    LocalPciRegister |= (1 << CoreNum);
  ...
  )
{
  ...
  if (((((F15_NB_PSTATE_CTRL_REGISTER *) &LocalPciRegister)->NbPstateMaxVal != 0) || SkipHwCfg)
      && (PlatformConfig->PlatformProfile.NbPstatesSupported)) {
    return TRUE;
  }
  return FALSE;
}
