 ...
 * @file
 *
 * AMD Family 15h Carrizo IO C-state feature support functions.
 *
...
CODE_GROUP (G3_DXE)
RDATA_GROUP (G3_DXE)
#define FILECODE PROC_CPU_FAMILY_0X15_CZ_CPUF15CZIOCSTATE_FILECODE
...
VOID
STATIC
F15CzInitializeIoCstateOnCore (
  IN       VOID *CstateBaseMsr,
  IN       AMD_CONFIG_PARAMS *StdHeader
  );

BOOLEAN
F15CzIsCsdObjGenerated (
...
/*---------------------------------------------------------------------------------------*/
/**
 *  Enable IO Cstate on a family 15h Carrizo CPU.
  ...
  )
{
  ...
  if ((EntryPoint & CPU_FEAT_AFTER_PM_INIT) != 0) {
    ...
    TaskPtr.FuncAddress.PfApTaskI = F15CzInitializeIoCstateOnCore;
...
/*---------------------------------------------------------------------------------------*/
/**
 *  Enable CState on a family 15h Carrizo core.
 *
...
CONST IO_CSTATE_FAMILY_SERVICES ROMDATA F15CzIoCstateSupport =
{
  0,
