 ...
 * @file
 *
 * AMD Family 15h Stoney after warm reset sequence for core P-states
 *
...
#include "cpuF15StCoreAfterReset.h"
#include "Filecode.h"
CODE_GROUP (G3_DXE)
RDATA_GROUP (G3_DXE)


#define FILECODE PROC_CPU_FAMILY_0X15_ST_CPUF15STCOREAFTERRESET_FILECODE
...
VOID
STATIC
F15StPmCoreAfterResetPhase1OnCore (
  IN       AMD_CONFIG_PARAMS *StdHeader
  );

VOID
STATIC
F15StPmCoreAfterResetPhase2OnCore (
  IN       VOID              *HwPsMaxVal,
  IN       AMD_CONFIG_PARAMS *StdHeader
  );
...
/*---------------------------------------------------------------------------------------*/
/**
 * Family 15h Stoney core 0 entry point for performing the necessary steps for core
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StPmCoreAfterReset\n");

  PciAddress.AddressValue = CPTC2_PCI_ADDR;
  LibAmdPciRead (AccessWidth32, PciAddress, &HwPsMaxVal, StdHeader);
  HwPsMaxVal = ((F15_ST_CLK_PWR_TIMING_CTRL2_REGISTER *) &HwPsMaxVal)->HwPstateMaxVal;

  // Launch each local core to perform steps 1 through 3.
  TaskPtr.FuncAddress.PfApTask = F15StPmCoreAfterResetPhase1OnCore;
  ...
  TaskPtr.FuncAddress.PfApTaskI = F15StPmCoreAfterResetPhase2OnCore;
...
/*---------------------------------------------------------------------------------------*/
/**
 * Support routine for F15StPmCoreAfterReset to perform MSR initialization on all
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StPmCoreAfterResetPhase1OnCore\n");
  ...
  if (((COFVID_STS_MSR *) &CofvidSts)->CurPstateLimit != ((COFVID_STS_MSR *) &CofvidSts)->CurPstate) {
    while ((GetF15StCurCpuFid (&CofvidSts) != GetF15StCpuFid (&LocalMsrRegister)) ||
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StPmCoreAfterResetPhase2OnCore\n");
  ...
  } while ((GetF15StCurCpuFid (&LocalMsrRegister) != GetF15StCpuFid (&TargetPsMsr)) ||
  ...
  if (((COFVID_STS_MSR *) &LocalMsrRegister)->CurPstateLimit != ((COFVID_STS_MSR *) &LocalMsrRegister)->CurPstate) {
    do {
      LibAmdMsrRead (MSR_COFVID_STS, &LocalMsrRegister, StdHeader);
    } while (GetF15StCurCpuVid (&LocalMsrRegister) != GetF15StCpuVid (&TargetPsMsr));
  }
