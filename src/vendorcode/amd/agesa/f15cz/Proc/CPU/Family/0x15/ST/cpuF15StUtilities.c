 ...
 * @file
 *
 * AMD Family 15h ST models 70h - 7Fh specific utility functions.
 *
 * Provides numerous utility functions specific to family 15h ST.
 *
...
CODE_GROUP (G2_PEI)
RDATA_GROUP (G2_PEI)
#define FILECODE PROC_CPU_FAMILY_0X15_ST_CPUF15STUTILITIES_FILECODE

/*----------------------------------------------------------------------------------------
 *                   D E F I N I T I O N S    A N D    M A C R O S
 *----------------------------------------------------------------------------------------
 */
extern REGISTER_TABLE_AT_GIVEN_TP F15StRegTableListAtGivenTP[];
...
AGESA_STATUS
STATIC
F15StCalculateNbFrequencyInMHz (
  IN       UINT32                NbFid,
  IN       UINT32                NbDid,
...
VOID
STATIC
F15StGetNbPstateSelectionTable (
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StGetProcIddMax - P%d\n", Pstate);
  ...
  if (PstateMsr.PsEnable == 1) {
    F15StCmnCalculateCurrentInmA ((UINT32) PstateMsr.IddValue, (UINT32) PstateMsr.IddDiv, &ProcIddMaxPerCore, StdHeader);
...
/*---------------------------------------------------------------------------------------*/
/**
 * Set down core register on Stoney
 *
...
CONST CPU_CORE_LEVELING_FAMILY_SERVICES ROMDATA F15StCoreLeveling =
{
  0,
  F15StSetDownCoreRegister
};
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StGetCurrentNbFrequency\n");

  PciAddress.AddressValue = NB_PSTATE_STATUS_PCI_ADDR;
  LibAmdPciRead (AccessWidth32, PciAddress, &NbPstateStsReg, StdHeader);
  ReturnCode = F15StCalculateNbFrequencyInMHz (
                 GetF15StCurNbFid (&NbPstateStsReg),
  ...
  )
{
  ...
  for (i = 0; i <= NbPsMaxVal; i++) {
    ...
    if (((NB_PSTATE_REGISTER *) &LocalPciRegister)->NbPstateEn == 1) {
      AgesaStatus = F15StCalculateNbFrequencyInMHz (GetF15StNbFid (&LocalPciRegister),
  ...
  if (i > NbPsMaxVal) {
    ...
    LibAmdPciRead (AccessWidth32, *PciAddress, &LocalPciRegister, StdHeader);
    F15StCalculateNbFrequencyInMHz (GetF15StCurNbFid (&LocalPciRegister),
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StGetNbPstateInfo - NB P%d\n", NbPstate);
  ...
  if ((NbPstate == 0) || (FamilySpecificServices->IsNbPstateEnabled (FamilySpecificServices, PlatformConfig, StdHeader))) {
    ...
    if (NbPstate <= NbPstateCtrlReg.NbPstateMaxVal) {
      ...
      IDS_HDT_CONSOLE (CPU_TRACE, "    En:%d Fid:%x Did:%x Vid:%x\n", NbPstateReg.NbPstateEn, GetF15StNbFid (&NbPstateReg), NbPstateReg.NbDid, GetF15StNbVid (&NbPstateReg));
      ...
      if (NbPstateReg.NbPstateEn == 1) {
        ...
        ASSERT (GetF15StNbFid (&NbPstateReg) <= 0x2E);
        F15StGetNbFreqNumeratorInMHz (GetF15StNbFid (&NbPstateReg), FreqNumeratorInMHz, StdHeader);
        F15StGetNbFreqDivisor (NbPstateReg.NbDid, FreqDivisor, StdHeader);
        // Check for P-state Bandwidth Requirements on
        // "400MHz <= NBCOF <= 1600MHz"
        ASSERT ((*FreqNumeratorInMHz / *FreqDivisor) >= 400);
        ASSERT ((*FreqNumeratorInMHz / *FreqDivisor) <= 1600);

        NbVid = GetF15StNbVid (&NbPstateReg);
        F15StCovertVidInuV  (NbVid, VoltageInuV, StdHeader);
        PstateIsValid = TRUE;
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StGetNbIddMax - NB P%d\n", NbPstate);
  ...
  if (NbPstate <= NbPstateCtrlReg.NbPstateMaxVal) {
    ...
    // Ensure that requested NbPstate is enabled
    if (NbPstateReg.NbPstateEn == 1) {
      F15StCmnCalculateCurrentInmA (NbPstateReg.NbIddValue, NbPstateReg.NbIddDiv, NbIddMax, StdHeader);
      IsNbPsEnabled = TRUE;
  ...
  )
{
  ...
  IDS_SKIP_HOOK (IDS_NPST, NULL, StdHeader) {
    if (F15StIsNPstEnabled (StdHeader)) {
      IDS_HDT_CONSOLE (CPU_TRACE, "  NPST is enabled\n");

      // Get Nb-Pstate selection table based on TDP
      F15StGetNbPstateSelectionTable (&NbSelectionTable, &(CpuEarlyParamsPtr->PlatformConfig), StdHeader);

      // Collect all Nb-Pstate setting from FUSE
      F15StGetNbPstateFromFuse (&NbFuse[0], StdHeader);
      ...
      if ((NbSts.StartupNbPstate != NbCtrlOnEntry.NbPstateHi) &&
        ...
        F15StTransitionToNbLow (StdHeader);
        LibAmdPciRead (AccessWidth32, PciAddress, &NbSts, StdHeader);
      }
      ...
      for (i = 0; i < NM_NB_PS_REG; i++) {
        ...
        F15StModifyNbPstate (&NbFuse[0], i, *Selection, StdHeader);
        Selection++;
      }
      ...
      if (NbSts.CurNbPstateLo == 0) {
        // Current NbPstate is NbHi
        F15StTransitionToNbLow (StdHeader);
        F15StModifyNbPstate (&NbFuse[0], CurrentNbPstate, SelForCurrentNb, StdHeader);
        F15StTransitionToNbHigh (StdHeader);
      } else {
        F15StTransitionToNbHigh (StdHeader);
        F15StModifyNbPstate (&NbFuse[0], CurrentNbPstate, SelForCurrentNb, StdHeader);
        F15StTransitionToNbLow (StdHeader);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StNbPstateDis\n");
  ...
  if (!FamilySpecificServices->IsNbPstateEnabled (FamilySpecificServices, &CpuEarlyParamsPtr->PlatformConfig, StdHeader)) {
    ...
    F15StNbPstateDisCore (StdHeader);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StNbPstateDisCore\n");
...
{
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StGetNbFreqNumeratorInMHz - NbFid=%d\n", NbFid);
...
{
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StGetNbFreqDivisor - NbDid=%d\n", NbDid);
  *FreqDivisor = (1 << NbDid);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StCalculateNbFrequencyInMHz - NbFid=%x, NbDid=%x\n", NbFid, NbDid);

  ReturnStatus = AGESA_SUCCESS;
  F15StGetNbFreqNumeratorInMHz (NbFid, &FreqNumeratorInMHz, StdHeader);
  F15StGetNbFreqDivisor (NbDid, &FreqDivisor, StdHeader);
...
{
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StCovertVidInuV\n");
...
{
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StCmnGetIddDivisor - IddDiv=%d\n", IddDiv);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StCmnCalculateCurrentInmA - IddValue=%x, IddDiv=%x\n", IddValue, IddDiv);

  F15StCmnGetIddDivisor (IddDiv, &IddDivisor, StdHeader);
  ...
  )
{
  ...
  IgnoreService = NULL;
  SystemCoreNumber = F15StGetApCoreNumber (IgnoreService, StdHeader);
...
{
  return (REGISTER_TABLE_AT_GIVEN_TP *) &F15StRegTableListAtGivenTP;
}
