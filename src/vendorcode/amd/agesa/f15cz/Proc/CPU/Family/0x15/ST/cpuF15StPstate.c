 ...
 * @file
 *
 * AMD Family 15h Stoney Pstate feature support functions.
 *
...
#define FILECODE PROC_CPU_FAMILY_0X15_ST_CPUF15STPSTATE_FILECODE
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StIsPstatePsdDependent\n");

  // Family 15h Stoney defaults to dependent PSD; allow Platform Configuration to
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StGetPstateTransLatency\n");
  ...
  PStateMaxValueOnCurrentCore = ((F15_ST_CLK_PWR_TIMING_CTRL2_REGISTER *) &LocalPciRegister)->HwPstateMaxVal;
  ...
  for (k = 0; k <= PStateMaxValueOnCurrentCore; k++) {
    ...
    if (((PSTATE_MSR *) &MsrData)->PsEnable == 1) {
      CpuFid = (UINT32) GetF15StCpuFid (&MsrData);
      if (k == 0) {
        HwP0CpuFid = CpuFid;
      }
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StGetPstateFrequency - P%d\n", StateNumber);
  ...
  CpuFid = (UINT32) GetF15StCpuFid (&LocalMsrRegister);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StGetPstatePower - P%d\n", StateNumber);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StGetPstateMaxState\n");

  LocalPciRegister = 0;

  // For F15 Stoney CPU, skip boosted p-state. The boosted p-state number = F4x15C[NumBoostStates].
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F15StGetPstateRegisterInfo - P%d\n", PState);
...
CONST PSTATE_CPU_FAMILY_SERVICES ROMDATA F15StPstateServices =
{
  0,
