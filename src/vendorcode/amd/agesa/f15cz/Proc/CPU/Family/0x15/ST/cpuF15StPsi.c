 ...
 * @file
 *
 * AMD Family 15h ST PSI Initialization
 *
...
#define FILECODE PROC_CPU_FAMILY_0X15_ST_CPUF15STPSI_FILECODE
  ...
  )
{
  ...
  if ((EntryPoint & (CPU_FEAT_AFTER_POST_MTRR_SYNC | CPU_FEAT_AFTER_RESUME_MTRR_SYNC)) != 0) {
    ...
    IDS_HDT_CONSOLE (CPU_TRACE, "  F15StPmVrmLowPowerModeEnable\n");
    ...
    if (PlatformConfig->VrmProperties[NbVrm].LowPowerThreshold != 0) {
      ...
      for (NbPstate = 0; NbPstate <= NbPstateMaxVal; NbPstate++) {
        ...
        CurrentNbVid = (UINT32) GetF15StNbVid (&NbPstateReg);
...
CONST PSI_FAMILY_SERVICES ROMDATA F15StPsiSupport =
{
  0,
