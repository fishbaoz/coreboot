 ...
 * @e \$Revision: 315154 $   @e \$Date: 2015-03-20 16:10:36 +0800 (Fri, 20 Mar 2015) $
 *
 */
/*
 ******************************************************************************
...
#define FILECODE PROC_CPU_FAMILY_0X15_ST_CPUF15STBTC_FILECODE
  ...
  )
{
  ...
  TargetPstate = cpuF15StGetTargetPstate (BtcBufferPtr.TestNumber, StdHeader);
  F15TransitionPstate (NULL, TargetPstate, (BOOLEAN) TRUE, StdHeader);
  cpuF15StExecuteBTC (&BtcBufferPtr, StdHeader);
 ...
 */
VOID
cpuF15StBTCLaunchAps (
  IN       AP_EXE_PARAMS *ApExeParams
  )
{
  UINT8   TargetPstate;

  TargetPstate = cpuF15StGetTargetPstate (0, &ApExeParams->StdHeader);
...
//    TargetPstate = cpuF15StGetTargetPstate (BtcBufferPtr.TestNumber, &ApExeParams->StdHeader);
//    F15TransitionPstate (NULL, TargetPstate, (BOOLEAN) TRUE, &ApExeParams->StdHeader);
//    cpuF15StExecuteBTC (&BtcBufferPtr, &ApExeParams->StdHeader);
  ...
  )
{
  ...
  F15_ST_CLK_PWR_TIMING_CTRL2_REGISTER Cptc2;
  ...
  )
{
  ...
  for (i = 0; i < LOOP_COUNT; i++) {
    ExecuteF15StBtc (BtcBufferPtr->TestNumber, WorkBufferPtr);
  }
  ...
  )
{
  ...
  TargetPstate = cpuF15StGetTargetPstate (0, StdHeader);
  F15TransitionPstate (NULL, TargetPstate, (BOOLEAN) FALSE, StdHeader);

  ApParams.StdHeader = *StdHeader;
  ApParams.FunctionNumber = ((UINT32) RUN_BTC_ON_ALL_APS + (UINT32) AMD_FAMILY_15_ST);
...
CONST BTC_FAMILY_SERVICES ROMDATA F15StBtcSupport =
{
  0,
