  ...
  for (Core = StartCore; (Core <= EndCore) && (Core <= (NumberOfCores - 1)); Core++) {
    if (Core == 0) {
      ...
      if (TimePoint == IDS_AP_RUN_CODE_EARLY) {
        ...
        IdsRunCodeOnCoreEarly (0, Core, &ApTask, StdHeader);
      } else if (TimePoint == IDS_AP_RUN_CODE_POST) {
        OptionApCommConfiguration.ApRunCodeOnSocketCore (0, Core, &ApTask, StdHeader);
        ...
        GetLocalApicIdForCore (0, Core, &ApicIdOfCore, StdHeader);
...
{
/// @todo, PCI/MSR tables are updated
  ...
  )
{
  ...
  OptionApCommConfiguration.ApRunCodeOnSocketCore ((UINT8)Socket, PEarlyApTask->Ap_Task0.Core, &PEarlyApTask->Ap_Task0.ApTask, StdHeader);
}
  ...
  )
{
  ...
  if ((Socket == BscSocket) || (Core == 0)) {
    OptionApCommConfiguration.ApRunCodeOnSocketCore (Socket, Core, ApTask, StdHeader);
  } else {
    ...
    if ((ApTask->DataTransfer.DataSizeInDwords * sizeof (UINT32)) <= IDS_EARLY_AP_TASK_PARA_NUM) {
      ...
      OptionApCommConfiguration.ApRunCodeOnSocketCore (Socket, 0, &Core0Task, StdHeader);
    }
  }
}
