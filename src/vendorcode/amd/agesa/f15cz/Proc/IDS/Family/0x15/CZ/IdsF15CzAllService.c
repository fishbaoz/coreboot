...
#define FILECODE PROC_IDS_FAMILY_0X15_CZ_IDSF15CZALLSERVICE_FILECODE

/**
 *  IDS Backend Function for F15CZ workaround
 *
 *
 *  Parameters:
  ...
  )
{
  ...
  IDS_NV_READ_SKIP (NvValue, AGESA_IDS_NV_MSR_DE_CFG_BIT16, IdsNvPtr, StdHeader) {
    if (NvValue != 0xFF) {
      // not 'Auto'
      GetLogicalIdOfCurrentCore (&CpuFamilyRevision, StdHeader);
      if (CpuFamilyRevision.Revision == AMD_F15_CZ_A0) {
        LibAmdMsrRead (0xC0011029, &MsrValue, StdHeader);
        MsrValue &= ~BIT16;
        MsrValue |= (NvValue & 1) << 16;
        LibAmdMsrWrite (0xC0011029, &MsrValue, StdHeader);
      }
    }
  }
  return IDS_SUCCESS;
...
/**
 *  IDS Backend Function for Channel Interleave F15 CZ
 *
 ...
 **/
IDS_STATUS
IdsIntSubChannelInterleaveF15Cz (
  IN OUT   VOID *DataPtr,
  IN OUT   AMD_CONFIG_PARAMS *StdHeader,
  IN       IDS_NV_ITEM *IdsNvPtr
  )
...
CONST IDS_FAMILY_FEAT_STRUCT ROMDATA IdsFeatWorkAroundBlockF15Cz =
{
  IDS_FEAT_COMMON,
  IDS_ALL_CORES,
...
CONST IDS_FAMILY_FEAT_STRUCT ROMDATA IdsFeatMemoryMappingPostBeforeBlockF15Cz =
{
...
CONST IDS_FAMILY_FEAT_STRUCT ROMDATA IdsFeatMemoryMappingChIntlvBlockF15Cz =
{
  IDS_FEAT_MEMORY_MAPPING,
  IDS_ALL_CORES,
...
CONST IDS_FAMILY_FEAT_STRUCT ROMDATA IdsFeatGnbPlatformCfgBlockF15Cz =
{
...
CONST IDS_FAMILY_FEAT_STRUCT ROMDATA IdsFeatRegGmmxF15Cz =
