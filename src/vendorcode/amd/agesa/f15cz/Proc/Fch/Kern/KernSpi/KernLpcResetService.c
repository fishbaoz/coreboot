...
#define FILECODE PROC_FCH_KERN_KERNSPI_KERNLPCRESETSERVICE_FILECODE
#define SPI_BASE UserOptions.FchBldCfg->CfgSpiRomBaseAddress

/**
 * FchInitKernResetLpcPciTable - Lpc (Spi) device registers
 ...
 *
 */
REG8_MASK FchInitKernResetLpcPciTable[] =
{
  //
  ...
  )
{
  ...
  ProgramPciByteTable ( (REG8_MASK*) (&FchInitKernResetLpcPciTable[0]), sizeof (FchInitKernResetLpcPciTable) / sizeof (REG8_MASK), StdHeader);
  ...
  )
{
  ...
  if (ACPIMMIO8 (0xFED8060C) == 0x20) {
    if (LocalCfgPtr->Mode == 0) {
      LocalCfgPtr->Mode = FCH_SPI_MODE_QUAL_144;
    }
    if (FchConfigureSpiDeviceDummyCycle ( (UINT32) (*(UINT8*) (UINTN) (0xFED8060C)), LocalCfgPtr->Mode)) {
      return TRUE;
    }
  }
