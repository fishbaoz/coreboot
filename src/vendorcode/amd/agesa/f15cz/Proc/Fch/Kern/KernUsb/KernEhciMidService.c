 ...
 * @file
 *
 * Config KERN FCH USB EHCI controller
 *
  ...
  )
{
  ...
  if ( (BarAddress != - 1) && (BarAddress != 0) ) {
    ...
    // 1. Program EHCI[3:1]xB4[PortNumber] with the controller port number according to Table 108.
    // 2. Program EHCI[3:1]xB4[VControlModeSel]=1000b to select Group 8.
    // 3. Program EHCI[3:1]xB4[VControl[4]]=0b and EHCI[3:1]xB4[VControl[5]]=1b to use (Actual+Adjusted) CalAmp value for drive strength.
    // 4. Program EHCI[3:1]xB4[VControl[3:0]] with desired drive strength adjustment.
    // 5. Program EHCI[3:1]xB4[VLoadB]=1.
    // 6. Wait for EHCI[3:1]xB4[VBusy]=0.
    // 7. Program EHCI[3:1]xB4[VLoadB]=0.
    // 8. Wait for EHCI[3:1]xB4[VBusy]=0.
    // 9. Program EHCI[3:1]xB4[VLoadB]=1 to lock the PHY control interface.
    for (PortNum = 0; PortNum < 4; PortNum ++) {
      DrivingStrength = (UINT32) (LocalCfgPtr->Usb.Ehci1Phy[PortNum]);
      DrivingStrength &= 0xF;
      DrivingStrength |= BIT5;
      RwMem (BarAddress + FCH_EHCI_BAR_REGB4, AccessWidth32, 0xFFFFC000, (PortNum << 13) + BIT12 + BIT10 + DrivingStrength);
      Index = 0;
      do {
    ...
    RwPci ((UINT32) Value + FCH_EHCI_REG50, AccessWidth32, ~ ((UINT32) (0x01 << 4)), (UINT32) (0x01 << 4), StdHeader);
    ...
    // (EHCI_REG54 BIT14) Enable CsplitDatatglResoEnhance
    RwPci ((UINT32) Value + FCH_EHCI_REG54, AccessWidth16, ~(UINT32) BIT14, BIT14, StdHeader);
...
//EHCI_PCI_74h[14] = 0
//EHCI_PCI_74h[8] = 1
    RwPci ((UINT32) Value + FCH_EHCI_REG74, AccessWidth16, ~(UINT32) ( BIT12 + BIT7 + BIT14 + BIT8 ), ( BIT12 + BIT7 + BIT8 ), StdHeader);
    RwPci ((UINT32) Value + FCH_EHCI_REG74, AccessWidth8, ~(UINT32) (BIT2 + BIT5 ), BIT5, StdHeader);
    RwPci ((UINT32) Value + FCH_EHCI_REG74, AccessWidth8, ~(UINT32) ( BIT6 ), BIT6, StdHeader);
//EHCI_PCI_74h[3] = 1 //ENH454495:[CZ] Set EHCI_PCI_74h[3] to one to fix EHCI SMI storm issue
  ...
  if (LocalCfgPtr->Usb.UsbBatteryChargeEnable) {
    // EHCI Battery Charge
    RwMem (BarAddress + FCH_EHCI_BAR_REGDC, AccessWidth32, ~((UINT32) (0x0F)), (UINT32) (0xF));
    // XHCI Battery Charge
    RwMem (FCH_XHC_PMx60_xHC_Battery_Charger_Enable, AccessWidth32, ~((UINT32) (0x0F)), (UINT32) (0xF));
  }
}
