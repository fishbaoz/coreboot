...
#include "Filecode.h"
#include "KernFch.h"
#define FILECODE PROC_FCH_KERN_KERNSATA_KERNIDE2AHCIENV_FILECODE
  ...
  )
{
  ...
  if (LocalCfgPtr->Sata.SataAhciSsid != NULL ) {
    ...
    ReadPci ((SATA_BUS_DEV_FUN << 16) + FCH_SATA_REG00, AccessWidth32, &SataDID, StdHeader);
    RwPci ((SATA_BUS_DEV_FUN << 16) + FCH_SATA_REG2C, AccessWidth32, 0x00, SataDID, StdHeader);
  }
