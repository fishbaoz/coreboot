...
#include "KernFch.h"
#include "Filecode.h"
#define FILECODE PROC_FCH_KERN_KERNIMC_KERNIMCENV_FILECODE
  ...
  )
{
  ...
  // Software IMC enable
  //
  if (((LocalCfgPtr->Imc.ImcEnableOverWrite == 1) || (LocalCfgPtr->Imc.ImcEnable == 1)) && (ValidateImcFirmware (LocalCfgPtr))) {
    ...
    ACPIMMIO8 (FCH_AOACx7A_IMC_D3_CONTROL) |= BIT3;
    ACPIMMIO8 (FCH_AOACx7A_IMC_D3_CONTROL) &= ~ (BIT0 + BIT1);
    FchStall (1000, StdHeader);
    ACPIMMIO16 (FCH_PMxD6_Imc_Gating) |= BIT15;
    ACPIMMIO8 (FCH_IOMUX_55_FANOUT0_AGPIO85) &= 0xFC;
    LocalCfgPtr->Imc.ImcEnabled = 1;
    ...
    //if ( LocalCfgPtr->EcChannel0 == ENABLED) {
      //
      //Logical device 0x03
      //
