 ...
 * mmflowd4st.c
 *
 * Main Memory initialization sequence for DDR4 only for ST
 *
...
#define FILECODE PROC_MEM_MAIN_ST_MMFLOWD4ST_FILECODE
/* features */
#include "mftds.h"
 ...
 *      systems that only support DDR4 and ST processor.
 *
  ...
  )
{
  ...
  //----------------------------------------------------------------
  MemNClockAndPowerGateUnusedDctST (NBPtr);

  //----------------------------------------------------------------
  // Set DDR4 mode
  //----------------------------------------------------------------
  MemNSetDdrModeD4ST (NBPtr);
      ...
      if (MemTSPDSetBanks4 (NBPtr->TechPtr)) {
        if (MemNStitchMemoryNb (NBPtr)) {
          if (NBPtr->DCTPtr->Timings.CsEnabled == 0) {
            ...
            MemNProgramDqMappingsD4ST (NBPtr);
            IDS_HDT_CONSOLE (MEM_FLOW, "\t\tAuto Config\n");
            MemNAutoConfigST (NBPtr);
            IDS_HDT_CONSOLE (MEM_FLOW, "\t\tSet Training Config\n");
            MemNConfigureDctForTrainingD4ST (NBPtr);
          }
        }
      }
    }
  ...
  for (Dct = 0; Dct < NBPtr->DctCount; Dct++) {
    ...
    if (NBPtr->DCTPtr->Timings.DctMemSize != 0) {
      ...
      MemNSetPhyDdrModeST (NBPtr, DRAM_TYPE_DDR4_ST);
    }
  }
  ...
  for (MemPstate = LowestMemPstate; MemPstate >= 0; MemPstate--) {
    ...
    IDS_HDT_CONSOLE (MEM_FLOW, "\n");
    MemNSwitchMemPstateST (NBPtr, MemPstate);
    ...
    for (Dct = 0; Dct < NBPtr->DctCount; Dct++) {
      ...
      if (NBPtr->DCTPtr->Timings.DctMemSize != 0) {
        ...
        if (MemTAutoCycTiming4 (NBPtr->TechPtr)) {
          ...
          MemNProgramMemPstateRegD4ST (NBPtr, MemPstate);
          IDS_HDT_CONSOLE (MEM_FLOW, "\t\tProgram Platform Specific Values\n");
          if (MemNPlatformSpecST (NBPtr)) {
            ...
            MemNProgramCadDataBusD4ST (NBPtr);

            IDS_HDT_CONSOLE (MEM_FLOW, "\t\tPredriver Init\n");
            MemNPredriverInitD4ST (NBPtr);

            IDS_HDT_CONSOLE (MEM_FLOW, "\t\tMode Register Initialization\n");
            MemNModeRegisterInitializationD4ST (NBPtr);

            // Enable Phy Power Savings for DDR4 after testing is complete
            IDS_HDT_CONSOLE (MEM_FLOW, "\t\tDRAM PHY Power Savings\n");
            MemNDramPhyPowerSavingsST (NBPtr, DRAM_TYPE_DDR4_ST);
  ...
  for (Dct = 0; Dct < NBPtr->DctCount; Dct++) {
    ...
    if (NBPtr->DCTPtr->Timings.DctMemSize != 0) {
      ...
      MemNPhyGenCfgST (NBPtr);

      // 5. Phy Voltage Level Programming
      MemNPhyVoltageLevelST (NBPtr, DRAM_TYPE_DDR4_ST);

      // 7. Program DRAM channel frequency
      MemNProgramChannelFreqST (NBPtr, DRAM_TYPE_DDR4_ST);

      // Step 8 and 9 are done in MemPs dependent section

      // 10. Program FIFO pointer init values
      MemNPhyFifoConfigD4ST (NBPtr);
    }
  }

  MemFInitTableDrive (NBPtr, MTBeforeTrn);

  IEM_INSERT_CODE (IEM_EARLY_DEVICE_INIT, IemEarlyDeviceInitD4ST, (NBPtr));
  ...
  for (Dct = 0; Dct < NBPtr->DctCount; Dct++) {
    MemNSwitchDCTNb (NBPtr, Dct);
    if (NBPtr->DCTPtr->Timings.DctDimmValid != 0) {
      MemNEnablePhyCalibrationST (NBPtr);
    }
  }
  ...
  if (MemNInitPmuSramMsgBlockST (NBPtr) == FALSE) {
  ...
  for (PmuImage = 0; PmuImage < NBPtr->PmuFirmwareImageTableSize; PmuImage++) {
    ...
    //
    if (MemNCheckPmuFirmwareImageST (NBPtr)) {
      ...
      for (Dct = 0; Dct < NBPtr->DctCount; Dct++) {
        ...
        if (NBPtr->DCTPtr->Timings.DctMemSize != 0) {
          ...
          for (MemPstate = LowestMemPstate; MemPstate >= 0; MemPstate--) {
            ...
            MemNPopulatePmuSramTimingsD4ST (NBPtr);
          }

          MemNPopulatePmuSramConfigD4ST (NBPtr);
          MemNSetPmuSequenceControlST (NBPtr);
          IEM_INSERT_CODE (IEM_BEFORE_PMU_MSG_BLOCK_WRITE, IemBeforePmuSramMsgBlockWriteST, (NBPtr));
          if (MemNWritePmuSramMsgBlockST (NBPtr) == FALSE) {
          ...
          MemNPendOnPhyCalibrateCompletionST (NBPtr);
      ...
      for (Dct = 0; Dct < NBPtr->DctCount; Dct++) {
        ...
        if (NBPtr->DCTPtr->Timings.DctMemSize != 0) {
          ...
          //
          MemNReadPmuSramMsgBlockST (NBPtr);
          if (PmuResult == FALSE) {
            //
            // PMU Training Failure
            //
            AGESA_TESTPOINT (TpProcMemPmuFailed, &(MemMainPtr->MemPtr->StdHeader));
            CSTestFail = ((PMU_SRAM_MSG_BLOCK_ST*)NBPtr->PsPtr->PmuSramMsgBlockPtr)->CsTestFail;
          ...
          if (PmuImage == 1) {
            ...
            for (MemPstate = LowestMemPstate; MemPstate >= 0; MemPstate--) {
              ...
              MemNSetBitFieldNb (NBPtr, BFMxMr6, (MemPstate == 0) ?
                                 ((PMU_SRAM_MSG_BLOCK_ST*)NBPtr->PsPtr->PmuSramMsgBlockPtr)->Annex.D4u.M0_MR6 :
                                 ((PMU_SRAM_MSG_BLOCK_ST*)NBPtr->PsPtr->PmuSramMsgBlockPtr)->Annex.D4u.M1_MR6);
          ...
          MemNRateOfPhyCalibrateST (NBPtr);
        } // Dct Memsize
      } // Wait all channels
    } // Check PMU Firmware Image
  }
  ...
  //----------------------------------------------------------------
  IDS_HDT_CONSOLE (MEM_FLOW, "\nDIMM Excludes\n");
  MemNDimmExcludesST (NBPtr);

  IEM_INSERT_CODE (IEM_LATE_DCT_CONFIG, IemLateDctConfigD4ST, (NBPtr));

  //----------------------------------------------------------------
  //  Synchronize Channels
  //----------------------------------------------------------------
  MemNSyncChannelInitST (NBPtr);
  ...
  IEM_SKIP_CODE (IEM_LATE_MAX_RD_LAT) {
    ...
    NBPtr->TechPtr->ResetDCTWrPtr = MemNResetRcvFifoST;
    ...
    for (i = 0; i < TimesMrlRetrain; i ++) {
      ...
      MemTTrainMaxLatencyST (NBPtr->TechPtr);
  ...
  for (Dct = 0; Dct < NBPtr->DctCount; Dct++) {
    ...
    if (NBPtr->DCTPtr->Timings.DctMemSize != 0) {
      ...
      MemNConfigureDctNormalD4ST (NBPtr);
      ...
      for (MemPstate = LowestMemPstate; MemPstate >= 0; MemPstate--) {
        IDS_HDT_CONSOLE (MEM_FLOW, "\t");
        MemNSwitchMemPstateST (NBPtr, MemPstate);
        MemNProgramTurnaroundTimingsD4ST (NBPtr);
  ...
  IEM_INSERT_CODE (IEM_LATE_MAX_RD_LAT, IemLateMaxRdLatD4ST, (NBPtr));
  IDS_HDT_CONSOLE (MEM_FLOW, "\nAdditional DRAM PHY Power Savings\n");
  for (Dct = 0; Dct < NBPtr->DctCount; Dct++) {
    MemNSwitchDCTNb (NBPtr, Dct);
    if (NBPtr->DCTPtr->Timings.DctMemSize != 0) {
      MemNAddlDramPhyPowerSavingsST (NBPtr);
    }
  }
  ...
  //----------------------------------------------------------------
  MemNInitChannelIntlvAddressBitST (NBPtr);
  ...
  MemNAllocateC6AndAcpEngineStorageST (NBPtr);
  ...
  if (MemNPostPmuSramMsgBlockST (NBPtr) == FALSE) {
