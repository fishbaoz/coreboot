...
#define FILECODE PROC_MEM_MAIN_MMLVDDR4_FILECODE
  ...
  )
{
  ...
  if (mmSharedPtr->VoltageMap == 0) {
    ...
    ParameterPtr->DDRVoltage = CONVERT_ENCODED_TO_VDDIO (LibAmdBitScanReverse (mmSharedPtr->VoltageMap), DDR4_TECHNOLOGY);
  }
  ...
  )
{
  ...
  mmSharedPtr = MemMainPtr->mmSharedPtr;
  ParameterPtr = MemMainPtr->MemPtr->ParameterListPtr;
  PowerPolicy = MemMainPtr->MemPtr->PlatFormConfig->PlatformProfile.MemoryPowerPolicy;
