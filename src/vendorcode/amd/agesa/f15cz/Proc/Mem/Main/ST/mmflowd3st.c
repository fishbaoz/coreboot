 ...
 * mmflowd3ST.c
 *
 * Main Memory initialization sequence for DDR3 only for ST
 *
...
#define FILECODE PROC_MEM_MAIN_ST_MMFLOWD3ST_FILECODE
/* features */
#include "mftds.h"
 ...
 *      systems that only support DDR3 and ST processor.
 *
  ...
  )
{
      ...
      if (MemTSPDSetBanks3 (NBPtr->TechPtr)) {
        if (MemNStitchMemoryNb (NBPtr)) {
          if (NBPtr->DCTPtr->Timings.CsEnabled == 0) {
            ...
            MemNConfigureDctForTrainingD3ST (NBPtr);
          }
        }
      }
    }
  ...
  for (Dct = 0; Dct < NBPtr->DctCount; Dct++) {
    ...
    if (NBPtr->DCTPtr->Timings.DctMemSize != 0) {
      ...
      MemNSetPhyDdrModeST (NBPtr, DRAM_TYPE_DDR3_ST);
    }
  }
    ...
    for (Dct = 0; Dct < NBPtr->DctCount; Dct++) {
      ...
      if (NBPtr->DCTPtr->Timings.DctMemSize != 0) {
        ...
        if (MemTAutoCycTiming3 (NBPtr->TechPtr)) {
          ...
          MemNProgramMemPstateRegD3ST (NBPtr, MemPstate);
          ...
          if (MemNPlatformSpecST (NBPtr)) {
            ...
            MemNProgramCadDataBusD3ST (NBPtr);

            IDS_HDT_CONSOLE (MEM_FLOW, "\t\tPredriver Init\n");
            MemNPredriverInitD3ST (NBPtr);

            IDS_HDT_CONSOLE (MEM_FLOW, "\t\tMode Register Initialization\n");
            MemNModeRegisterInitializationD3ST (NBPtr);

            IDS_HDT_CONSOLE (MEM_FLOW, "\t\tDRAM PHY Power Savings\n");
            MemNDramPhyPowerSavingsST (NBPtr, DRAM_TYPE_DDR3_ST);
          }
        }
      }
    }
  ...
  for (Dct = 0; Dct < NBPtr->DctCount; Dct++) {
    ...
    if (NBPtr->DCTPtr->Timings.DctMemSize != 0) {
      ...
      MemNPhyVoltageLevelST (NBPtr, DRAM_TYPE_DDR3_ST);

      // 7. Program DRAM channel frequency
      MemNProgramChannelFreqST (NBPtr, DRAM_TYPE_DDR3_ST);

      // Step 8 and 9 are done in MemPs dependent section

      // 10. Program FIFO pointer init values
      MemNPhyFifoConfigD3ST (NBPtr);
    }
  }

  MemFInitTableDrive (NBPtr, MTBeforeTrn);

  IEM_INSERT_CODE (IEM_EARLY_DEVICE_INIT, IemEarlyDeviceInitD3ST, (NBPtr));
      ...
      for (Dct = 0; Dct < NBPtr->DctCount; Dct++) {
        ...
        if (NBPtr->DCTPtr->Timings.DctMemSize != 0) {
          ...
          for (MemPstate = LowestMemPstate; MemPstate >= 0; MemPstate--) {
            ...
            MemNPopulatePmuSramTimingsD3ST (NBPtr);
          }

          MemNPopulatePmuSramConfigD3ST (NBPtr);
  ...
  IEM_INSERT_CODE (IEM_LATE_DCT_CONFIG, IemLateDctConfigD3ST, (NBPtr));
  ...
  for (Dct = 0; Dct < NBPtr->DctCount; Dct++) {
    ...
    if (NBPtr->DCTPtr->Timings.DctMemSize != 0) {
      ...
      MemNConfigureDctNormalD3ST (NBPtr);
      ...
      for (MemPstate = LowestMemPstate; MemPstate >= 0; MemPstate--) {
        IDS_HDT_CONSOLE (MEM_FLOW, "\t");
        MemNSwitchMemPstateST (NBPtr, MemPstate);
        MemNProgramTurnaroundTimingsD3ST (NBPtr);
  ...
  IEM_INSERT_CODE (IEM_LATE_MAX_RD_LAT, IemLateMaxRdLatD3ST, (NBPtr));
