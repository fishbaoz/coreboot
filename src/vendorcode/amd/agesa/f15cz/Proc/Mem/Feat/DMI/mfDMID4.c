...
#define FILECODE PROC_MEM_FEAT_DMI_MFDMID4_FILECODE
/*----------------------------------------------------------------------------
    ...
    for (Channel = 0; Channel < MaxChannelsPerSocket; Channel ++) {
      ...
      for (Dimm = 0; Dimm < MaxDimmsPerChannel; Dimm ++, SpdDataStructure ++, LocalHandle++) {
        ...
        if (SpdDataStructure->DimmPresent) {
          ...
          //
          ///
          /// @todo: Extended SPD Data is needed here
          ///
          ...
          DmiPhysicalDimmInfoTable->MinimumVoltage = CONVERT_ENCODED_TO_VDDIO_MILLIVOLTS (LibAmdBitScanReverse (VoltageMap), DDR4_TECHNOLOGY);
          DmiPhysicalDimmInfoTable->MaximumVoltage = CONVERT_ENCODED_TO_VDDIO_MILLIVOLTS (LibAmdBitScanForward (VoltageMap), DDR4_TECHNOLOGY);
