  ...
  if (TargetNbPs < 4) {
    ...
    if ((NBPtr->NbPsCtlReg & 0x00000003) != 0) {
      ...
      if ((TargetNbPs != 0) || !SkipTransToLo) {
        ...
        TempNbPs = (UINT8)MemNGetBitFieldNb (NBPtr, BFNbPstateHi);
        ...
        MemUWait10ns (100000, NBPtr->MemPtr);
        MemNPollBitFieldNb (NBPtr, BFCurNbPstateLo, 1, PCI_ACCESS_TIMEOUT, FALSE);
        MemNPollBitFieldNb (NBPtr, BFNbPstateReqBusy, 0, PCI_ACCESS_TIMEOUT, FALSE);
        MemNWaitForNbPstateChangeCompletion (NBPtr, NbPstateMaxVal);
      }
      ...
      MemNPollBitFieldNb (NBPtr, BFCurNbPstateLo, 0, PCI_ACCESS_TIMEOUT, FALSE);
      MemNPollBitFieldNb (NBPtr, BFNbPstateReqBusy, 0, PCI_ACCESS_TIMEOUT, FALSE);
      MemNWaitForNbPstateChangeCompletion (NBPtr, TargetNbPs);
    }
    ...
    if ((UINT8)MemNGetBitFieldNb (NBPtr, BFCurMemPstate) != NBPtr->MemPstate) {
