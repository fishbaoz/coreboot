 ...
 * mnpmust.c
 *
 * Support functions for memory training using PMU for ST
 *
...
#define FILECODE PROC_MEM_NB_ST_MNPMUST_FILECODE
...
{
  MemNLoadPmuFirmwareImageST (NBPtr);
}
  ...
  )
{
  ...
  if (NBPtr->PmuFirmwareImage == 1) {
    //
    // 2D Training Image (Skip if 1D or Auto Mode)
    //
    if ((NBPtr->RefPtr->PmuTrainMode == PMU_TRAIN_1D) ||
        (NBPtr->RefPtr->PmuTrainMode == PMU_TRAIN_AUTO)) {
      result = FALSE;
    }
  }
  return result;
  ...
  )
{
  ...
  ASSERT (PMU_SRAM_DMEM_SIZE_ST <= PMU_SRAM_DMEM_SIZE_MAX);
  ASSERT (PMU_SRAM_IMEM_SIZE_ST <= PMU_SRAM_IMEM_SIZE_MAX);
  // Check the FW Image Table Pointer
  ASSERT (NBPtr->PmuFirmwareImage < PMU_IMAGE_MAX_ST);
  ...
  for (i = 0; i < PMU_SRAM_DMEM_SIZE_ST / sizeof (PmuFirmwareImagePtr->SramDataImage[0]); i++) {
  ...
  for (i = 0; i < PMU_SRAM_IMEM_SIZE_ST / sizeof (PmuFirmwareImagePtr->SramCodeImage[0]); i++) {
