 ...
 * mnregst.c
 *
 * Common Northbridge register related functions for ST
 *
...
#define FILECODE PROC_MEM_NB_ST_MNREGST_FILECODE
...
STATIC CONST DDR_CHIP_TO_PAD_ENTRY ROMDATA ChipToPadTableST[] = {
  {MEMCKE0, 2, 0, 0, 3},
  {MEMCKE0, 3, 0, 0, 2},
  {MEMCKE0, 0, 0, 0, 1},
  {MEMCKE0, 1, 0, 0, 0},
 ...
 *
 *     @return          TRUE -  This node is a ST.
 *     @return          FALSE - This node is not a ST.
 *
 */
BOOLEAN
MemNIsIdSupportedST (
...
{
  if (((LogicalIdPtr->Family & AMD_FAMILY_15_ST) != 0)
  ...
  )
{
  ...
  PadTabIdx = GET_SIZE_OF (ChipToPadTableST);
  UseGroup = FALSE;
  UsePadTab = FALSE;
  ...
  if (NBPtr->NBRegTable[FieldName] != 0) {
    ...
    do {
      if (RegIndex > DctPhyRegRangeStart) {
        ...
        while (PadTabIdx < GET_SIZE_OF (ChipToPadTableST)) {
          if (BfNameEnc.Parse.PadName == ChipToPadTableST[PadTabIdx].PadName) {
            if (((Address & 0x000F0000) == 0x00010000) || (BfNameEnc.Parse.PadIdxPlus1 == 0) || (BfNameEnc.Parse.PadIdxPlus1 == (UINT8) (ChipToPadTableST[PadTabIdx].Instance + 1))) {
              BroadcastIndex[1] = BroadcastLimit[1] = UseGroup ? ChipToPadTableST[PadTabIdx].Group : ChipToPadTableST[PadTabIdx].PadNum;
              if ((Address & 0x000F0000) == 0x00000000) {
                // Only use table chiplet for ABYTE. DBYTE chiplets are byte lanes.
                BroadcastIndex[2] = BroadcastLimit[2] = (UINT8) ChipToPadTableST[PadTabIdx].Chiplet;
              }
          ...
          PadTabIdx++;
          if (PadTabIdx == GET_SIZE_OF (ChipToPadTableST)) {
        ...
        if (!IsSet || !WholeRegAccess) {
          ...
          MemNCmnGetSetFieldST (NBPtr, 1, RegDctAddlOffset, ReadAddress);
          Value = MemNCmnGetSetFieldST (NBPtr, 0, RegDctAddlData, 0);
      ...
      if (IsSet) {
        ...
        if (RegIndex > DctPhyRegRangeStart) {
          ...
          MemNCmnGetSetFieldST (NBPtr, 1, RegDctAddlData, Value);
          MemNCmnGetSetFieldST (NBPtr, 1, RegDctAddlOffset, Address | DCT_ACCESS_WRITE);
        ...
        if (IsLinked) {
          MemNCmnGetSetFieldST (NBPtr, 1, FieldName + 1, InputValue >> (HiBit - LoBit + 1));
        }
        ...
        if (IsLinked) {
          Value |= MemNCmnGetSetFieldST (NBPtr, 0, FieldName + 1, 0) << (HiBit - LoBit + 1);
        }
      }
  ...
  )
{
  ...
  LocHeap.BufferHandle = GENERATE_MEM_HANDLE (ALLOC_NB_REG_TABLE, NbRegTabST, 0, 0);
  ...
  AllocHeapParams.BufferHandle = GENERATE_MEM_HANDLE (ALLOC_NB_REG_TABLE, NbRegTabST, 0, 0);
  ...
  _BF_DEF (NBRegTable, RegDramCtlrSelHi, BFDctSelBankSwap,  9,  9);
