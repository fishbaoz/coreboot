...
#define FILECODE PROC_MEM_NB_ST_MNST_FILECODE
/*----------------------------------------------------------------------------
  ...
  )
{
  ...
  if (!MemNIsIdSupportedST (&(MemPtr->DiesPerSystem->LogicalCpuid))) {
    return FALSE;
  }
  ...
  //
  AllocHeapParams.RequestedBufferSize = MAX_DCTS_PER_NODE_ST * (
                                          sizeof (DCT_STRUCT) + (
                                            MAX_CHANNELS_PER_DCT_ST * (sizeof (CH_DEF_STRUCT) + sizeof (MEM_PS_BLOCK) + sizeof (CH_TIMING_STRUCT))
                                          )
                                        );
  ...
  MCTPtr->DctCount = MAX_DCTS_PER_NODE_ST;
  MCTPtr->DctData = (DCT_STRUCT *) AllocHeapParams.BufferPtr;
  AllocHeapParams.BufferPtr += MAX_DCTS_PER_NODE_ST * sizeof (DCT_STRUCT);
  for (Dct = 0; Dct < MAX_DCTS_PER_NODE_ST; Dct++) {
    MCTPtr->DctData[Dct].Dct = Dct;
    MCTPtr->DctData[Dct].ChannelCount = MAX_CHANNELS_PER_DCT_ST;
    MCTPtr->DctData[Dct].ChData = (CH_DEF_STRUCT *) AllocHeapParams.BufferPtr;
    MCTPtr->DctData[Dct].ChData[0].Dct = Dct;
    AllocHeapParams.BufferPtr += MAX_CHANNELS_PER_DCT_ST * sizeof (CH_DEF_STRUCT);
    MCTPtr->DctData[Dct].TimingsMemPs1 = (CH_TIMING_STRUCT *) AllocHeapParams.BufferPtr;
    AllocHeapParams.BufferPtr += MAX_CHANNELS_PER_DCT_ST * sizeof (CH_TIMING_STRUCT);
  }
  ...
  //
  for (Dct = 0; Dct < MAX_DCTS_PER_NODE_ST; Dct++) {
    for (Channel = 0; Channel < MAX_CHANNELS_PER_DCT_ST; Channel++) {
  ...
  )
{
  ...
  NBPtr->ChannelCount = MAX_CHANNELS_PER_DCT_ST;
  ...
  NBPtr->CsPerChannel = MAX_CS_PER_CHANNEL_ST;
  ...
  NBPtr->MemNcmnGetSetTrainDly = MemNcmnGetSetTrainDlyST;
  NBPtr->StitchMemory = MemNStitchMemoryNb;
  NBPtr->DisableDCT = MemNDisableDctST;
  NBPtr->HtMemMapInit = MemNHtMemMapST;
  ...
  NBPtr->MemNCapSpeedBatteryLife = MemNCapSpeedBatteryLifeST;
  ...
  NBPtr->GetMemClkFreqInCurrentContext = MemNGetMemClkFreqInCurrentContextST;
  NBPtr->ProgramNbPsDependentRegs = MemNProgramNbPstateDependentRegistersST;

  NBPtr->GetUmaSize = MemNGetUmaSizeST;
  NBPtr->UMAMemTyping = MemNUMAMemTypingNb;

  NBPtr->MemNDisableScrubber = MemNDisableScrubberST;
  NBPtr->MemNRestoreScrubber = MemNRestoreScrubberST;
  ...
  NBPtr->MemS3PspDetect = MemS3PspDetectionST;
  NBPtr->MemS3PspPlatformSecureBootEn = MemS3PspPlatformSecureBootEnST;
  ...
  NBPtr->getMaxLatParams = MemNGetMaxLatParamsST;
  ...
  // ---------------------------------------------------------------------------
  NBPtr->FamilySpecificHook[DCTSelectSwitch] = MemNDctCfgSelectST;
  NBPtr->FamilySpecificHook[SetupHwTrainingEngine] = MemNSetupHwTrainingEngineUnb;
  NBPtr->FamilySpecificHook[ReleaseNbPstate] = MemNReleaseNbPstateST;
  NBPtr->FamilySpecificHook[FixupUmaInfo] = MemNFixupUmaInfoST;

  NBPtr->FamilySpecificHook[ForceEccSymbolSize] = MemNForceEccSymbolSizeST;
  NBPtr->FamilySpecificHook[DisableMemHoleMapping] = MemNDisableMemHoleMappingST;
  NBPtr->FamilySpecificHook[RestoreMemHoleMapping] = MemNRestoreMemHoleMappingST;
  NBPtr->FamilySpecificHook[DramScrubErratum792] = MemNDramScrubErratum792ST;
  NBPtr->FamilySpecificHook[GetDdrMaxRate] = MemNGetDdrMaxRateST;
  // ---------------------------------------------------------------------------
  ...
  )
{
  ...
  // Only initialize on ST processors, otherwise exit.
...
/**
 *
 *      This is function gets the erratum 792 for ST to disable ScrubRate and ScrubRedir
 *
 *     @param[in,out]   *NBPtr      - Pointer to the MEM_NB_BLOCK
