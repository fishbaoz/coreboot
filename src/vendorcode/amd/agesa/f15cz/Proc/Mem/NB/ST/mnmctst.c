 ...
 * mnmctst.c
 *
 * Northbridge ST MCT supporting functions
 *
...
#define FILECODE PROC_MEM_NB_ST_MNMCTST_FILECODE
/*----------------------------------------------------------------------------
...
/**
 *
 *     This function switches memory pstate for ST
 *
...
/**
 *
 *  This function calculates and programs NB P-state dependent registers for ST
 *
 *     @param[in,out]   *NBPtr   - Pointer to the MEM_NB_BLOCK
 *
 */
 ...
 *
 *
 *   This function releases the NB P-state force for ST
 *
 *     @param[in,out]   *NBPtr   - Pointer to the MEM_NB_BLOCK
  ...
  )
{
  ...
  if (NBPtr->RefPtr->UmaVersion == UMA_LEGACY) {
    ...
    if (SysMemSize >= 0x20000) {
      ...
      //
      switch (NBPtr->MemPtr->PlatFormConfig->Resolution) {
  ...
  )
{
  ...
  if ((IsFeatureEnabled (C6Cstate, NBPtr->MemPtr->PlatFormConfig, &(NBPtr->MemPtr->StdHeader)) || MemPspDetectionMechanismST (NBPtr)) && AcpEngineFeature) {
  ...
  // Only CC6 is enabled.
  } else if (IsFeatureEnabled (C6Cstate, NBPtr->MemPtr->PlatFormConfig, &(NBPtr->MemPtr->StdHeader)) || MemPspDetectionMechanismST (NBPtr))  {
  ...
  )
{
  ...
  // Set LockDramCfg
  if (IsFeatureEnabled (C6Cstate, NBPtr->MemPtr->PlatFormConfig, &(NBPtr->MemPtr->StdHeader)) || MemPspDetectionMechanismST (NBPtr)) {
