 ...
 * mnphyst.c
 *
 * Northbridge Phy support for ST
 *
...
#define FILECODE PROC_MEM_NB_ST_MNPHYST_FILECODE
/* features */
#include "mftds.h"
/*----------------------------------------------------------------------------
  ...
  )
{
  ...
  if ((Mode == DRAM_TYPE_DDR4_ST) && (NBPtr->ChannelPtr->Dimms > 1)) {
    Value8 = 0x5F;
  } else {
    Value8 = 0x40;
  }
  IDS_HDT_CONSOLE(MEM_FLOW, "RegVrefHSpeed = %02x\n", Value8);
  MemNSetBitFieldNb (NBPtr, PER_MEM_PS ((Mode == DRAM_TYPE_DDR4_ST) ? ALL_MEM_PS : 1, RegVrefHSpeed), Value8);
  ...
  )
{
  ...
  MemNSetBitFieldNb (NBPtr, RegPllMemPs1, (Mode == DRAM_TYPE_DDR3_ST) ? 0x0603 : 0x0D01);
