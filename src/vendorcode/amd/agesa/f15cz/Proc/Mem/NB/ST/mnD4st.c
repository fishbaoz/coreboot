 ...
 * mnD4st.c
 *
 * DDR4 Specific Northbridge functions for ST
 *
...
#define FILECODE PROC_MEM_NB_ST_MND4ST_FILECODE

extern ROMDATA PMU_FIRMWARE PmuFirmwareD4ImageTableST[];
extern ROMDATA UINT8 PmuFirmwareD4ImageTableSizeST;
extern MEM_PSC_FLOW_BLOCK memPlatSpecFlowD4ST;
/*----------------------------------------------------------------------------
  ...
  )
{
  ...
  //
  NBPtr->PmuFirmwareImageTable = PmuFirmwareD4ImageTableST;
  NBPtr->PmuFirmwareImageTableSize = PmuFirmwareD4ImageTableSizeST;
  //
  // Initialize Platform Specific Flow Block
  //
  NBPtr->PlatSpecFlowPtr = (VOID*) &memPlatSpecFlowD4ST;
  // ---------------------------------------------------------------------------
  // Common functions
  // ---------------------------------------------------------------------------
  ...
  // Misc functions
  // ---------------------------------------------------------------------------
  NBPtr->ProgramCycTimings = MemNProgramCycTimingsD4ST;
  NBPtr->GetCSIntLvAddr = MemNGetCSIntLvAddrD4ST;
  ...
  )
{
  ...
  for (Dct = 0; Dct < MAX_DCTS_PER_NODE_ST; Dct++) {
    ...
    MemNSetBitFieldNb (NBPtr, BFDramType, DRAM_TYPE_DDR4_ST);
  }

  // After disable unused DCTs, change the number of DCTs to 2 for DDR4 mode
  NBPtr->DctCount = MAX_D4_DCTS_PER_NODE_ST;
