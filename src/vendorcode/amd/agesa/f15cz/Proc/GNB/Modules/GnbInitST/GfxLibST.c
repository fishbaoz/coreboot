...
#include  "Filecode.h"
#define FILECODE PROC_GNB_MODULES_GNBINITST_GFXLIBST_FILECODE
/*----------------------------------------------------------------------------------------
 ...
 */
UINT32
GfxLibGetSystemPllCofST (
  IN       AMD_CONFIG_PARAMS      *StdHeader
  )
{
  D0F0xBC_xC001400C_STRUCT  D0F0xBC_xC001400C;
  GnbRegisterReadST (GnbGetHandle (StdHeader), D0F0xBC_xC001400C_TYPE,  D0F0xBC_xC001400C_ADDRESS, &D0F0xBC_xC001400C.Value, 0, StdHeader);
  ...
  )
{
  UINT32  Divider;
  UINT32  SystemPllCof;
  SystemPllCof = GfxLibGetSystemPllCofST (StdHeader) * 100;
  if (Did >= 8 && Did <= 0x3F) {
    Divider = Did * 25;
  ...
  )
{
  ...
  GnbRegisterReadST (GnbGetHandle (GnbLibGetHeader (Gfx)), GMMx1740_TYPE, GMMx1740_ADDRESS, &GMMx1740.Value, 0, GnbLibGetHeader (Gfx));
