 ...
 * @e project:     AGESA
 * @e sub-project: GNB
 * @e \$Revision: 313865 $   @e \$Date: 2015-03-02 15:34:25 +0800 (Mon, 02 Mar 2015) $
 *
 */
/*
*****************************************************************************
...
#include  "Filecode.h"
#define FILECODE PROC_GNB_MODULES_GNBINITST_PCIEEARLYINITST_FILECODE
/*----------------------------------------------------------------------------------------
 *                   D E F I N I T I O N S    A N D    M A C R O S
 *----------------------------------------------------------------------------------------
 */
extern BUILD_OPT_CFG UserOptions;
extern CONST PCIE_HOST_REGISTER_TABLE_HEADER ROMDATA CoreInitTableST;
extern CONST PCIE_HOST_REGISTER_TABLE_HEADER ROMDATA PcieInitEarlyTableST;
extern CONST PCIE_HOST_REGISTER_TABLE_HEADER ROMDATA PhyLaneInitEarlyTableST;
extern CONST PCIE_HOST_REGISTER_TABLE_HEADER ROMDATA PhyWrapperInitEarlyTableST;
extern CONST PCIE_PORT_REGISTER_TABLE_HEADER ROMDATA PortInitEarlyTableST;
...
VOID
STATIC
PcieTopologyPrepareForReconfigST (
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieTopologyPrepareForReconfigST Enter\n");
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieTopologyPrepareForReconfigST Exit\n");
}
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieTopologySetCoreConfigST Enter\n");
  if (PcieLibIsPcieWrapper (Wrapper)) {
    for (CoreId = Wrapper->StartPcieCoreId; CoreId <= Wrapper->EndPcieCoreId; CoreId++) {
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieTopologySetCoreConfigST Exit\n");
  return Status;
}

UINT8 LaneMuxSelectorArrayST[] = { 0, 1, 2, 3, 4, 5, 6, 7 };
...
UINT8
STATIC
PcieTopologyLocateMuxIndexST (
  IN OUT   UINT8                *LaneMuxSelectorArrayPtr,
  IN       UINT8                LaneMuxValue
  )
{
  UINT8  Index;
  for (Index = 0; Index < sizeof (LaneMuxSelectorArrayST); Index++ ) {
  ...
  )
{
  ...
  UINT32              Index;
  UINT8               RxLaneMuxSelectorArray [sizeof (LaneMuxSelectorArrayST)];
  UINT8               TxLaneMuxSelectorArray [sizeof (LaneMuxSelectorArrayST)];

  IDS_HDT_CONSOLE (GNB_TRACE, "PcieTopologyApplyLaneMuxST Enter\n");
  if (PcieLibIsPcieWrapper (Wrapper)) {
    LibAmdMemCopy (
      &TxLaneMuxSelectorArray[0],
      &LaneMuxSelectorArrayST[0],
      sizeof (LaneMuxSelectorArrayST),
      ...
      if (PcieLibIsPcieEngine (EngineList) && PcieLibIsEngineAllocated (EngineList)) {
        ...
        while (CoreLaneBitmap != 0) {
          ...
          if (TxLaneMuxSelectorArray[CurrentPifLane] != CurrentCoreLane) {
            TxLaneMuxSelectorArray[PcieTopologyLocateMuxIndexST (TxLaneMuxSelectorArray, CurrentCoreLane)] = TxLaneMuxSelectorArray[CurrentPifLane];
            TxLaneMuxSelectorArray[CurrentPifLane] = CurrentCoreLane;
          }

          if (RxLaneMuxSelectorArray[CurrentCoreLane] != CurrentPifLane) {
            RxLaneMuxSelectorArray[PcieTopologyLocateMuxIndexST (RxLaneMuxSelectorArray, CurrentPifLane)] = RxLaneMuxSelectorArray[CurrentCoreLane];
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieTopologyApplyLaneMuxST Exit\n");
}
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieTopologySelectMasterPllST Enter\n");
  ...
  //
  // ST PCIE master pll always select PHY0 (0 <= lane <= 7)
  //
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieTopologySelectMasterPllST Exit\n");
}
  ...
  )
{
  ...
  if (PcieLibIsPcieWrapper (Wrapper)) {
    IDS_HDT_CONSOLE (GNB_TRACE, "PcieTopologyExecuteReconfigST Enter\n");
    ...
    IDS_HDT_CONSOLE (GNB_TRACE, "PcieTopologyExecuteReconfigST Exit\n");
  }
}
  ...
  )
{
  D0F0xE4_CORE_0101_STRUCT  D0F0xE4_CORE_0101;
  UINT8                     CoreId;
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieTopologyCleanUpReconfigST Enter\n");
  if (PcieLibIsPcieWrapper (Wrapper)) {
    for (CoreId = Wrapper->StartPcieCoreId; CoreId <= Wrapper->EndPcieCoreId; CoreId++) {
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieTopologyCleanUpReconfigST Exit\n");
}
...
AGESA_STATUS
STATIC
PciePortMapInitCallbackST (
  IN       PCIe_DESCRIPTOR_HEADER            *Descriptor,
  IN OUT   VOID                              *Buffer,
  IN       PCIe_PLATFORM_CONFIG              *Pcie
  )
{
  PcieSetPortPciAddressMapST ((PCIe_SILICON_CONFIG *) Descriptor);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PciePhyLaneInitInitCallbackST Enter\n");
  ...
  for (Lane = 0; Lane < Wrapper->NumberOfLanes; ++Lane) {
    ...
    if ((LaneBitmap & (1 << Lane)) != 0) {
      for (Index = 0; Index < PhyLaneInitEarlyTableST.Length; Index++) {
        UINT32 Value;
        Value = PcieRegisterRead (
                  Wrapper,
                  PHY_SPACE (Wrapper->WrapId, Phy, PhyLaneInitEarlyTableST.Table[Index].Reg + (PhyLaneIndex * 0x100)),
                  Pcie
                  );
        Value &= (~PhyLaneInitEarlyTableST.Table[Index].Mask);
        Value |= PhyLaneInitEarlyTableST.Table[Index].Data;
        PcieRegisterWrite (
          Wrapper,
          PHY_SPACE (Wrapper->WrapId, Phy, PhyLaneInitEarlyTableST.Table[Index].Reg + (PhyLaneIndex * 0x100)),
          Value,
  ...
  for (Lane = 0; Lane < Wrapper->NumberOfLanes; Lane += MAX_NUM_LANE_PER_PHY) {
    Phy = Lane / MAX_NUM_LANE_PER_PHY;
    for (Index = 0; Index < PhyWrapperInitEarlyTableST.Length; Index++) {
      UINT32 Value;
      Value = PcieRegisterRead (
                Wrapper,
                PHY_SPACE (Wrapper->WrapId, Phy, PhyWrapperInitEarlyTableST.Table[Index].Reg),
                Pcie
                );
      Value &= (~PhyWrapperInitEarlyTableST.Table[Index].Mask);
      Value |= PhyWrapperInitEarlyTableST.Table[Index].Data;
      PcieRegisterWrite (
        Wrapper,
        PHY_SPACE (Wrapper->WrapId, Phy, PhyWrapperInitEarlyTableST.Table[Index].Reg),
        Value,
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PciePhyLaneInitInitCallbackST Exit\n");
  return AGESA_SUCCESS;
}
  ...
  )
{
  ...
  if (PcieLibIsPcieWrapper (Wrapper)) {
    IDS_HDT_CONSOLE (GNB_TRACE, "PcieEarlyCoreInitST Enter\n");
    for (CoreId = Wrapper->StartPcieCoreId; CoreId <= Wrapper->EndPcieCoreId; CoreId++) {
      for (Index = 0; Index < CoreInitTableST.Length; Index++) {
        UINT32 Value;
        Value = PcieRegisterRead (
                  Wrapper,
                  CORE_SPACE (CoreId, CoreInitTableST.Table[Index].Reg),
                  Pcie
                  );
        Value &= (~CoreInitTableST.Table[Index].Mask);
        Value |= CoreInitTableST.Table[Index].Data;
        PcieRegisterWrite (
          Wrapper,
          CORE_SPACE (CoreId, CoreInitTableST.Table[Index].Reg),
          Value,
    ...
    IDS_HDT_CONSOLE (GNB_TRACE, "PcieEarlyCoreInitST Exit\n");
  }
}
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieHwInitPowerGatingST Enter\n");
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieHwInitPowerGatingST Exit\n");
}
...
VOID
STATIC
PcieEarlyWrapperTxPresetLoadingSequenceST (
  IN       PCIe_WRAPPER_CONFIG           *Wrapper,
  IN       PCIe_PLATFORM_CONFIG          *Pcie
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieEarlyWrapperTxPresetLoadingSequenceST Enter\n");
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieEarlyWrapperTxPresetLoadingSequenceST Exit\n");
}
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieEarlyInitCallbackST Enter\n");
  ...
  PcieTopologyPrepareForReconfigST (Wrapper, Pcie); //step 2
  Status = PcieTopologySetCoreConfigST (Wrapper, &CoreConfigChanged, Pcie); //step 3
  ASSERT (Status == AGESA_SUCCESS);
  PcieTopologyApplyLaneMuxST (Wrapper, Pcie); //step 4
//  PciePifSetRxDetectPowerMode (Wrapper, Pcie);
//  PciePifSetLs2ExitTime (Wrapper, Pcie);
//  PciePifApplyGanging  (Wrapper, Pcie);
//  PciePhyApplyGangingST (Wrapper, Pcie);
  PcieTopologySelectMasterPllST (Wrapper, &PllConfigChanged, Pcie); //step 5
  if (CoreConfigChanged) {
    PcieTopologyExecuteReconfigST (Wrapper, Pcie); // step 6
  }
  PcieEarlyWrapperTxPresetLoadingSequenceST (Wrapper, Pcie);
  PcieTopologyCleanUpReconfigST (Wrapper, Pcie); // step 7
  PcieTopologySetLinkReversalV4 (Wrapper, Pcie); // step 8
//  PciePifPllConfigureST (Wrapper, Pcie);
  PcieTopologyLaneControlST (
    DisableLanes,
...
//  PciePhyAvertClockPickersST (Wrapper, Pcie);
  PcieEarlyCoreInitST (Wrapper, Pcie);
  PcieSetSsidST (UserOptions.CfgGnbPcieSSID, Wrapper, Pcie);
  PcieHwInitPowerGatingST (Wrapper, Pcie);
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieEarlyInitCallbackST Exit [%x]\n", Status);
  return Status;
}
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieEarlyInitST Enter\n");
  AgesaStatus = AGESA_SUCCESS;
  Status = PcieConfigRunProcForAllDescriptors (DESCRIPTOR_SILICON, 0, DESCRIPTOR_TERMINATE_TOPOLOGY, PciePortMapInitCallbackST, NULL, Pcie);
  AGESA_STATUS_UPDATE (Status, AgesaStatus);

  // Remove temporarily for ST emulation.
  // Status = PcieConfigRunProcForAllWrappers (DESCRIPTOR_ALL_WRAPPERS, PciePhyLaneInitInitCallbackST, NULL, Pcie);
  // AGESA_STATUS_UPDATE (Status, AgesaStatus);

  Status = PcieConfigRunProcForAllWrappers (DESCRIPTOR_ALL_WRAPPERS, PcieEarlyInitCallbackST, NULL, Pcie);

  // Hide all Dev3 Bridges
  for (FunctionIndex = 1 ;FunctionIndex < 6; FunctionIndex++) {
    GnbLibPciIndirectRMW (
      D0F0xC8_ADDRESS,
      D0F0xCC_x01_ADDRESS | ((3 << 3 | FunctionIndex) << D0F0xC8_NB_DEV_IND_SEL_OFFSET),
      AccessWidth32,
      0xFFFFFFFFul,
      ((1 << D0F0xCC_x01_BridgeDis_OFFSET) | (1 << D0F0xCC_x01_CfgDis_OFFSET)),
      GnbLibGetHeader (Pcie)
    );
  }
  // Disable PCIe core 1
  ...
  AGESA_STATUS_UPDATE (Status, AgesaStatus);
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieEarlyInitST Exit [%x]\n", AgesaStatus);
  return  AgesaStatus;
}
...
VOID
STATIC
PcieEarlyPortInitCallbackST (
  IN       PCIe_ENGINE_CONFIG    *Engine,
  IN OUT   VOID                  *Buffer,
  IN       PCIe_PLATFORM_CONFIG  *Pcie
  )
{
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieEarlyPortInitCallbackST Enter\n");
  ASSERT (Engine->EngineData.EngineType == PciePortEngine);
  PciePortProgramRegisterTable (PortInitEarlyTableST.Table, PortInitEarlyTableST.Length, Engine, FALSE, Pcie);
  ...
  PcieLinkInitHotplugST (Engine, Pcie);
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieEarlyPortInitCallbackST Exit\n");
}
  ...
  )
{
  ...
  AgesaStatus = AGESA_SUCCESS;
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieEarlyInterfaceST Enter\n");
  Status = PcieLocateConfigurationData (StdHeader, &Pcie);
  AGESA_STATUS_UPDATE (Status, AgesaStatus);
  if (Status == AGESA_SUCCESS) {
    PciePortsVisibilityControlV5 (UnhidePorts, Pcie);

    Status = PcieEarlyInitST (Pcie);
    AGESA_STATUS_UPDATE (Status, AgesaStatus);
    ASSERT (Status == AGESA_SUCCESS);

    Status = PcieEarlyPortInitST (Pcie);
    AGESA_STATUS_UPDATE (Status, AgesaStatus);
    ASSERT (Status == AGESA_SUCCESS);
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieEarlyInterfaceST Exit [0x%x]\n", AgesaStatus);
  return  AgesaStatus;
}
