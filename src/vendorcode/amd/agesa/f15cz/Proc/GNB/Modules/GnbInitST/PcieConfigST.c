...
#include  "Filecode.h"
#define FILECODE PROC_GNB_MODULES_GNBINITST_PCIECONFIGST_FILECODE
/*----------------------------------------------------------------------------------------
...
extern PCIe_LANE_ALLOC_DESCRIPTOR ROMDATA   PcieLaneAllocConfigurationST[];
...
AGESA_STATUS
PcieConfigurePcieEnginesLaneAllocationST (
...
AGESA_STATUS
PcieConfigureDdiEnginesLaneAllocationST (
  IN      PCIe_WRAPPER_CONFIG           *Wrapper,
...
//
UINT8 ROMDATA DefaultPortDevMapST [] = {
  DEVFUNC (2, 1),
  DEVFUNC (2, 2),
  DEVFUNC (2, 3),
  DEVFUNC (2, 4),
...
APIC_DEVICE_INFO ROMDATA DefaultIoapicConfigST [] = {
  {0, 0, 0x14},
  {1, 0, 0x15},
  {2, 0, 0x16},
  {3, 0, 0x17},
  ...
  )
{
  ...
  Status = AGESA_ERROR;
  LaneConfigDescriptor = PcieLaneAllocConfigurationST;
  while (LaneConfigDescriptor != NULL) {
    if (LaneConfigDescriptor->WrapId == Wrapper->WrapId && LaneConfigDescriptor->EngineType == EngineType) {
      switch (EngineType) {
      case PciePortEngine:
        Status = PcieConfigurePcieEnginesLaneAllocationST (Wrapper, LaneConfigDescriptor, ConfigurationId);
        break;
      case PcieDdiEngine:
        Status = PcieConfigureDdiEnginesLaneAllocationST (Wrapper, LaneConfigDescriptor, ConfigurationId);
        break;
      default:
        ASSERT (FALSE);
      }
  ...
  )
{
  ...
  default:
    IDS_HDT_CONSOLE (PCIE_MISC, "ERROR!!![%s Wrapper] Unknown core config signature 0x%08x%08x\n",
      PcieDebugGetWrapperNameStringST (Wrapper),
  ...
  )
{
  ...
  for (Index = 0; Index < (sizeof (DefaultPortDevMapST) / sizeof (DefaultPortDevMapST[0])); Index++) {
    if (DefaultPortDevMapST[Index] == DevFunc) {
      return TRUE;
    }
  }
  return FALSE;
...
{
  ///@todo ST STRAP_BIF_LINK_CONFIG has not been confirmed
...
{
  AGESA_STATUS            Status;
  ST_COMPLEX_CONFIG       *ComplexConfig;
  PCIe_COMPLEX_CONFIG     *Complex;
  PCIe_PLATFORM_CONFIG    *Pcie;
  UINT8                   DevFunc;
  UINT8                   Index;
  Status = AGESA_SUCCESS;
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieMapPortPciAddressST Enter\n");
  ...
  ComplexConfig = (ST_COMPLEX_CONFIG *) PcieConfigGetParentSilicon (Engine);
  ...
  for (Index = 0; Index < (sizeof (DefaultPortDevMapST) / sizeof (DefaultPortDevMapST[0])); ++Index) {
    if (DevFunc == DefaultPortDevMapST[Index]) {
      ...
      if (Engine->Type.Port.PortData.ApicDeviceInfo.GroupMap == 0x00) {
        ...
        Engine->Type.Port.PortData.ApicDeviceInfo = DefaultIoapicConfigST[Index];
      }
      break;
    }
  }
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieMapPortPciAddressST Exit [0x%x]\n", Status);
  return  Status;
}
...
VOID
PcieSetPortPciAddressMapST (
  IN      PCIe_SILICON_CONFIG     *Silicon
  )
{
  UINT8                   Index;
  UINT8                   DevFuncIndex;
  UINT8                   PortDevMap [sizeof (DefaultPortDevMapST)];
  PCIe_PLATFORM_CONFIG    *Pcie;
  D0F0x64_x30_STRUCT      D0F0x64_x30;

  Pcie = (PCIe_PLATFORM_CONFIG *) PcieConfigGetParent (DESCRIPTOR_PLATFORM, &Silicon->Header);
  LibAmdMemCopy (&PortDevMap[0], &DefaultPortDevMapST[0], sizeof (DefaultPortDevMapST), GnbLibGetHeader (Pcie));
  for (Index = 0; Index < sizeof (((ST_COMPLEX_CONFIG *) Silicon)->FmSilicon.PortDevMap); ++Index) {
    if (((ST_COMPLEX_CONFIG *) Silicon)->FmSilicon.PortDevMap[Index] != 0) {
      for (DevFuncIndex = 0; DevFuncIndex < sizeof (((ST_COMPLEX_CONFIG *) Silicon)->FmSilicon.PortDevMap); ++DevFuncIndex) {
        if (PortDevMap[DevFuncIndex] == ((ST_COMPLEX_CONFIG *) Silicon)->FmSilicon.PortDevMap[Index]) {
    ...
    if (((ST_COMPLEX_CONFIG *) Silicon)->FmSilicon.PortDevMap[Index] == 0) {
      for (DevFuncIndex = 0; DevFuncIndex < sizeof (((ST_COMPLEX_CONFIG *) Silicon)->FmSilicon.PortDevMap); ++DevFuncIndex) {
        if (PortDevMap[DevFuncIndex] != 0) {
          ((ST_COMPLEX_CONFIG *) Silicon)->FmSilicon.PortDevMap[Index] = PortDevMap[DevFuncIndex];
          PortDevMap[DevFuncIndex] = 0;
          break;
        }
      }
    }
    GnbRegisterReadST ((GNB_HANDLE *) Silicon, D0F0x64_x30_TYPE, D0F0x64_x30_ADDRESS + Index, &D0F0x64_x30.Value, 0, GnbLibGetHeader (Pcie));
    D0F0x64_x30.Field.PBr0_DevFnMap = ((ST_COMPLEX_CONFIG *) Silicon)->FmSilicon.PortDevMap[Index];
    GnbRegisterWriteST ((GNB_HANDLE *) Silicon, D0F0x64_x30_TYPE, D0F0x64_x30_ADDRESS + Index, &D0F0x64_x30.Value, 0, GnbLibGetHeader (Pcie));
  }
}



PCIe_PORT_DESCRIPTOR DefaultSbPortST = {
  0,
  ...
  )
{
  ...
  if (((CpuId.EBX_Reg >> 28) & 0xF) == ST_SOCKET_FP4) {
    IDS_HDT_CONSOLE (GNB_TRACE, "ST FP4 Internal FCH\n");
    return AGESA_UNSUPPORTED;
  }
  // FM2 cantain 1x4 UMI
  LibAmdMemCopy (SbPort, &DefaultSbPortST, sizeof (DefaultSbPortST), StdHeader);
