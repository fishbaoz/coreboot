...
#include  "Filecode.h"
#define FILECODE PROC_GNB_MODULES_GNBINITCZ_GNBURACZ_FILECODE
  ...
  )
{
  ...
  case TYPE_GNB_PROTOCOL_ACCESS:
    ...
    GnbRegisterReadCZ (Device->GnbHandle, (UINT8)TargetAddress, UraTokenInfo->RegAddress, &RegValue, UraTokenInfo->Flags, Device->StdHeader);
  ...
  )
{
  ...
  case TYPE_GNB_PROTOCOL_ACCESS:
    if (UraTokenInfo->WholeRegAccess == TRUE) {
      ...
      GnbRegisterReadCZ (Device->GnbHandle, UraTokenInfo->RegDomainType, UraTokenInfo->RegAddress, &RegValue, UraTokenInfo->Flags, Device->StdHeader);
      FieldMask = (((UINT32)1 << UraTokenInfo->BfWidth) - 1);
      TargetValue = RegValue & (~(FieldMask << UraTokenInfo->BfOffset));
      TargetValue |= (TempValue & FieldMask) << UraTokenInfo->BfOffset;
    }
    GnbRegisterWriteCZ (Device->GnbHandle, UraTokenInfo->RegDomainType, UraTokenInfo->RegAddress, &TargetValue, UraTokenInfo->Flags, Device->StdHeader);
  ...
  )
{
  ...
  for (Index = 0; Index < CombinedCount; Index++) {
    ...
    case TYPE_GNB_PROTOCOL_ACCESS:
      TargetAddress = UraTokenInfo->RegDomainType;
      GnbRegisterWriteCZ (Device->GnbHandle, (UINT8)TargetAddress, StreamSetAddress, &RegValue, UraTokenInfo->Flags, Device->StdHeader);
      break;
