...
#include  "GnbRegistersST.h"
#include  "PcieComplexDataST.h"
#include  "PcieAriInitST.h"
#include  "GnbGfxInitLibV1.h"
#include  "Filecode.h"
#define FILECODE PROC_GNB_MODULES_GNBINITST_PCIEMIDINITST_FILECODE
/*----------------------------------------------------------------------------------------
 *                   D E F I N I T I O N S    A N D    M A C R O S
 *----------------------------------------------------------------------------------------
 */

extern CONST PCIE_PORT_REGISTER_TABLE_HEADER ROMDATA PortInitMidTableST;

#define ABS(X) (((X) < 0) ? (-(X)) : (X))

//for SOI PHY, NB L1 existing latency is about 30us
//for TSMC PHY, NB L1 existing latency is about 35us
#define NB_L1_EXITING_LATENCY_ST   35
...
VOID
STATIC
PcieMidAspmInitCallbackST (
  IN       PCIe_ENGINE_CONFIG    *Engine,
  IN OUT   VOID                  *Buffer,
  IN       PCIe_PLATFORM_CONFIG  *Pcie
  );

GNB_PARITY_TABLE GnbParityTableST[] = {
  ...
  )
{
  ...
  PciePortProgramRegisterTable (PortInitMidTableST.Table, PortInitMidTableST.Length, Engine, TRUE, Pcie);
...
VOID
STATIC
PciePwrAutoPowerDownElectricalIdleDetectorST (
  IN       PCIe_WRAPPER_CONFIG     *Wrapper,
  IN       PCIe_PLATFORM_CONFIG    *Pcie
  )
{
  UINT8   Pif;
  IDS_HDT_CONSOLE (GNB_TRACE, "PciePwrAutoPowerDownElectricalIdleDetectorST Enter\n");
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PciePwrAutoPowerDownElectricalIdleDetectorST Exit\n");
}
  ...
  )
{
  ...
  PcieSetVoltageST (GlobalSpeedCap, Pcie);
  return Status;
}
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PciePwrClockGatingST Enter\n");
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PciePwrClockGatingST Exit\n");
}
...
AGESA_STATUS
STATIC
PcieMidInitCallbackST (
  IN       PCIe_WRAPPER_CONFIG           *Wrapper,
  IN OUT   VOID                          *Buffer,
  IN       PCIe_PLATFORM_CONFIG          *Pcie
  )
{
  PciePwrAutoPowerDownElectricalIdleDetectorST (Wrapper, Pcie);
  PciePwrPowerDownUnusedLanesST (Wrapper, Pcie);
  PciePwrClockGatingST (Wrapper, Pcie);
  ...
  )
{
  ...
  AGESA_STATUS          Status;
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieMidInitST Enter\n");
  AgesaStatus = AGESA_SUCCESS;

  IDS_OPTION_HOOK (IDS_BEFORE_POWER_GATING, Pcie, GnbLibGetHeader (Pcie));
  Status = PcieConfigRunProcForAllWrappers (DESCRIPTOR_ALL_WRAPPERS, PcieMidInitCallbackST, NULL, Pcie);
  AGESA_STATUS_UPDATE (Status, AgesaStatus);

  Status = PciePowerGateST (Pcie);
  AGESA_STATUS_UPDATE (Status, AgesaStatus);

  IDS_HDT_CONSOLE (GNB_TRACE, "PcieMidInitST Exit [0x%x]\n", AgesaStatus);
  return  AgesaStatus;
}
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieAerInitST Enter\n");
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieAerInitST Exit\n");
  return;
}
  ...
  )
{
  ...
  for ( i = 0; i < sizeof (GnbParityTableST) / sizeof (GNB_PARITY_TABLE); i++) {
    D0F0xC4_x1.Value = 0x0;
    GnbRegisterWriteST (GnbHandle, D0F0xC4_x1_TYPE, D0F0xC4_x1_ADDRESS, &D0F0xC4_x1.Value, AccessWidth32, StdHeader);

    do {
      GnbRegisterReadST (GnbHandle, D0F0xC4_x1_TYPE, D0F0xC4_x1_ADDRESS, &D0F0xC4_x1.Value, 0, StdHeader);
    } while (D0F0xC4_x1.Field.ParityErrGenInjectAllow != 0x1);

    D0F0xC4_x1.Field.ParityErrGenGroupSel = GnbParityTableST[i].GroupID;
    if (GnbParityTableST[i].GroupType != PARITY_ECC_GROUP) {
      D0F0xC4_x1.Field.ParityErrGenGroupTypeSel = GnbParityTableST[i].GroupType;
    }
    D0F0xC4_x1.Field.ParityErrGenIdSel = GnbParityTableST[i].StructureID;
    D0F0xC4_x1.Field.ParityErrGenCmd = GnbParityTableST[i].ErrGenCmd;
...
VOID
STATIC
PcieMidPortCharacterizationCallbackST (
  IN       PCIe_ENGINE_CONFIG    *Engine,
  IN OUT   VOID                  *Buffer,
  IN       PCIe_PLATFORM_CONFIG  *Pcie
  )
...
AGESA_STATUS
STATIC
PcieMidWrapperCharacterizationCallbackST (
  IN       PCIe_WRAPPER_CONFIG   *Wrapper,
  IN       VOID                  *Buffer,
  IN       PCIe_PLATFORM_CONFIG  *Pcie
  )
{
  ...
  if ((WrapperInfo->MinAspmL1ExitLatency > NB_L1_EXITING_LATENCY_ST) && (Wrapper->Features.PllOffInL1 != 0) && (!WrapperInfo->AnyDevFailPllpdnb)) {
    IDS_HDT_CONSOLE (GNB_TRACE, "Minimum ASPM L1 Exit Latency on wrapper %x\n", WrapperInfo->MinAspmL1ExitLatency);
    IDS_HDT_CONSOLE (GNB_TRACE, "NB_L1_EXITING_LATENCY_ST %x\n", NB_L1_EXITING_LATENCY_ST);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieMidCharacterizationST Enter\n");
  AgesaStatus = AGESA_SUCCESS;
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieMidCharacterizationST Exit [0x%x]\n", AgesaStatus);
  return  AgesaStatus;
}
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieMidInterfaceST Enter\n");
  AgesaStatus = AGESA_SUCCESS;
  ...
  if (Status == AGESA_SUCCESS) {
    ...
    Status = PcieMidCharacterizationST (&PcieInfoBuffer, Pcie);
    AGESA_STATUS_UPDATE (Status, AgesaStatus);
    ASSERT (Status == AGESA_SUCCESS);

    Status = PcieMidPortInitST (Pcie);
    AGESA_STATUS_UPDATE (Status, AgesaStatus);
    ASSERT (Status == AGESA_SUCCESS);

    Status = PcieMidInitST (Pcie);
    AGESA_STATUS_UPDATE (Status, AgesaStatus);
    ASSERT (Status == AGESA_SUCCESS);

    Status = PcieAriInitST (StdHeader, Pcie);
    AGESA_STATUS_UPDATE (Status, AgesaStatus);
    ASSERT (Status == AGESA_SUCCESS);
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieMidInterfaceST Exit [0x%x]\n", AgesaStatus);
  return  AgesaStatus;
}
