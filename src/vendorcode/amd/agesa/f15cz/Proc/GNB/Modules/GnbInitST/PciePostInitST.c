...
#include  "Filecode.h"
#define FILECODE PROC_GNB_MODULES_GNBINITST_PCIEPOSTINITST_FILECODE
/*----------------------------------------------------------------------------------------
...
AGESA_STATUS
PciePostS3InterfaceST (
  IN      AMD_CONFIG_PARAMS               *StdHeader
  );

VOID
PcieLateRestoreInitSTS3Script (
  IN      AMD_CONFIG_PARAMS               *StdHeader,
...
VOID
STATIC
PciePhyLaneRxAdaptModeST (
  IN      PCIe_ENGINE_CONFIG    *Engine,
  IN      PCIe_PLATFORM_CONFIG  *Pcie
  )
{

  IDS_HDT_CONSOLE (GNB_TRACE, "PciePhyLaneInitRxAdaptModeST Enter\n");
  IDS_HDT_CONSOLE (GNB_TRACE, "PciePhyLaneInitRxAdaptModeST Exit\n");
}
...
//  PciePhyLaneRxAdaptModeST (Engine, Pcie);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PciePostInterfaceST Enter\n");
  ...
  if (Status == AGESA_SUCCESS) {
    ...
    Status = PciePostEarlyPortInitST (Pcie);
    AGESA_STATUS_UPDATE (Status, AgesaStatus);
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PciePostInterfaceST Exit [0x%x]\n", AgesaStatus);
  return  AgesaStatus;
}
  ...
  )
{
  ...
  if (Status == AGESA_SUCCESS) {
    ...
    Status = PciePostInitST (Pcie);
    AGESA_STATUS_UPDATE (Status, AgesaStatus);
    ASSERT (Status == AGESA_SUCCESS);

    Status = PciePostPortInitST (Pcie);
    AGESA_STATUS_UPDATE (Status, AgesaStatus);
    ASSERT (Status == AGESA_SUCCESS);
  ...
  )
{
  ...
  AGESA_STATUS          Status;
  PCIe_PLATFORM_CONFIG  *Pcie;
  IDS_HDT_CONSOLE (GNB_TRACE, "PciePostS3InterfaceST Enter\n");
  AgesaStatus = AGESA_SUCCESS;
  Status = PcieLocateConfigurationData (StdHeader, &Pcie);
  AGESA_STATUS_UPDATE (Status, AgesaStatus);
  if (Status == AGESA_SUCCESS) {
    ...
    if (Pcie->TrainingAlgorithm == PcieTrainingDistributed) {
      Status = PciePostS3PortInitST (Pcie);
    } else {
      Status = PciePostPortInitST (Pcie);
    }
    AGESA_STATUS_UPDATE (Status, AgesaStatus);
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PciePostS3InterfaceST Exit [0x%x]\n", AgesaStatus);
  return  AgesaStatus;
}
...
{
  PciePostS3InterfaceST (StdHeader);
}