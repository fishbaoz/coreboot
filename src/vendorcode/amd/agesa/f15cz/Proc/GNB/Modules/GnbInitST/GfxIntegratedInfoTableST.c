...
#include  "Filecode.h"
#define FILECODE PROC_GNB_MODULES_GNBINITST_GFXINTEGRATEDINFOTABLEST_FILECODE
/*----------------------------------------------------------------------------------------
...
/*----------------------------------------------------------------------------------------*/
CONST UINT8  DdiLaneConfigArrayST [][4] = {
  {16, 19, 0, 0},
  {19, 16, 0, 0},
  {20, 23, 1, 1},
  {23, 20, 1, 1},
  ...
  )
{
  ...
  for (DisplayPathIndex = 0; DisplayPathIndex <  (sizeof (DdiLaneConfigArrayST) / 4); DisplayPathIndex++) {
    if (DdiLaneConfigArrayST[DisplayPathIndex][0] == Engine->EngineData.StartLane &&
        DdiLaneConfigArrayST[DisplayPathIndex][1] == Engine->EngineData.EndLane) {
      PrimaryDisplayPathId = DdiLaneConfigArrayST[DisplayPathIndex][2];
      SecondaryDisplayPathId = DdiLaneConfigArrayST[DisplayPathIndex][3];
      break;
    }
  }
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "GfxFillNbPstateMemclkFreqST Enter\n");
  ...
  //For ST, change ulNbpStateMemclkFreq[4] to ulNbpStateMemclkFreq[2], just store hi and lo
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "GfxFillNbPstateMemclkFreqST Exit\n");
}
...
AGESA_STATUS
STATIC
GfxCalculateRestoreResetTimeST (
...
VOID
STATIC
GfxIntInfoTableInitTDPConfigST (
  IN       ATOM_INTEGRATED_SYSTEM_INFO_V1_9 *IntegratedInfoTable,
  IN       GFX_PLATFORM_CONFIG              *Gfx
  )
{
  ...
  if (TdpInfo == NULL) {
    IDS_HDT_CONSOLE (GNB_TRACE, "GfxIntInfoTableInitTDPConfigST failed!!\n");
    return;
  }
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "GfxIntegratedInfoTableInitST Enter\n");
  ...
  if (PpFuseArray != NULL) {
    ...
    // Complete table with ST-specific fields

    // Populate memory channels
    Status = GfxPopulateMemoryChannelsST (&SystemInfoTableV5.sIntegratedSysInfo, Gfx, PpFuseArray);
    AGESA_STATUS_UPDATE (Status, AgesaStatus);

    // Fill in Nb P-state MemclkFreq Data
    GfxFillNbPstateMemclkFreqST (&SystemInfoTableV5.sIntegratedSysInfo, PpFuseArray, Gfx);
    ...
    IDS_HDT_CONSOLE (GNB_TRACE, "ST Pplay done\n");

    // Assign usFormatID to represent ST
    PpTable->usFormatID = 0x17;
    PpTable->sThermalController.ucType = ATOM_PP_THERMALCONTROLLER_ST;

    // Family specific data update
    // Determine ulGMCRestoreResetTime
    Status = GfxCalculateRestoreResetTimeST (&SystemInfoTableV5.sIntegratedSysInfo, Gfx, PpFuseArray);
    AGESA_STATUS_UPDATE (Status, AgesaStatus);
    ...
    GfxIntInfoTableInitTDPConfigST (&SystemInfoTableV5.sIntegratedSysInfo, Gfx);
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "GfxIntegratedInfoTableInitST Exit [0x%x]\n", Status);
  return  AgesaStatus;
}
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "GfxIntegratedInfoTableInterfaceST Enter\n");
  AgesaStatus = AGESA_SUCCESS;
  if (GfxLibIsControllerPresent (StdHeader)) {
    Status = GfxLocateConfigData (StdHeader, &Gfx);
    AGESA_STATUS_UPDATE (Status, AgesaStatus);
    if (Status != AGESA_FATAL) {
      Status = GfxIntInfoTableInitST (Gfx);
      AGESA_STATUS_UPDATE (Status, AgesaStatus);
    }
  }
  IDS_HDT_CONSOLE (GNB_TRACE, "GfxIntegratedInfoTableInterfaceST Exit[0x%x]\n", AgesaStatus);
  return AgesaStatus;
}
