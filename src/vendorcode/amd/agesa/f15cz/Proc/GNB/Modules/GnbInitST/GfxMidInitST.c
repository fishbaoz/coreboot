...
#include  "Filecode.h"
#define FILECODE PROC_GNB_MODULES_GNBINITST_GFXMIDINITST_FILECODE
/*----------------------------------------------------------------------------------------
...
{
  IDS_HDT_CONSOLE (GNB_TRACE, "GfxSetBootUpVoltageST Enter\n");
///@todo voltage control
//  GfxRequestVoltageST (GnbLocateHighestVidCodeV5 (GnbLibGetHeader (Gfx)), GnbLibGetHeader (Gfx));
  IDS_HDT_CONSOLE (GNB_TRACE, "GfxSetBootUpVoltageST Enter\n");
  ...
  )
{
  ...
  if (UserOptions.CfgGnbIGPUSSID != 0) {
    ...
    // The next 2 digits are the ST RID
    // The least significant digit represents TDP.  0 and 1 represent 15W and 35W respectively
    // Example of 5-part ID ST 35W B10: VID=1002, DID=9874, SVID=OEM specific value, SSID=1C41, RID=C4(08/RevisonID)
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "GfxMidInterfaceST Enter\n");
  ...
  if (Status == AGESA_FATAL) {
    ...
    if (Gfx->UmaInfo.UmaMode != UMA_NONE) {
      ...
      Status = PcieEdpPortPowerCheckST (StdHeader);
      ASSERT (Status == AGESA_SUCCESS);
      AGESA_STATUS_UPDATE (Status, AgesaStatus);
      ...
      if (Status != AGESA_SUCCESS) {
        ...
        Status = GfxGmcInitST (Gfx);
        AGESA_STATUS_UPDATE (Status, AgesaStatus);

        Status = GfxSetBootUpVoltageST (Gfx);
        AGESA_STATUS_UPDATE (Status, AgesaStatus);

        Status = GfxInitSsidST (Gfx);
        AGESA_STATUS_UPDATE (Status, AgesaStatus);
        ...
        Status = GfxMakeHSAProcUnitEntryST (StdHeader);
        AGESA_STATUS_UPDATE (Status, AgesaStatus);
        ...
        if (AudioEPCount > GnbBuildOptionsST.GnbCommonOptions.CfgGnbNumDisplayStreamPipes) {
          AudioEPCount = GnbBuildOptionsST.GnbCommonOptions.CfgGnbNumDisplayStreamPipes;
        }

        AudioEPCount = 7 - AudioEPCount;
        GnbRegisterReadST (GnbGetHandle (StdHeader), GMMx60D0_TYPE, GMMx60D0_ADDRESS, &GMMx60D0.Value, 0, StdHeader);
        GMMx60D0.Field.PORT_CONNECTIVITY = AudioEPCount;
        GMMx60D0.Field.PORT_CONNECTIVITY_OVERRIDE_ENABLE = 1;
        GnbRegisterWriteST (GnbGetHandle (StdHeader), GMMx60D0_TYPE, GMMx60D0_ADDRESS, &GMMx60D0.Value, 0, StdHeader);
      }
    }
  }
  IDS_HDT_CONSOLE (GNB_TRACE, "GfxMidInterfaceST Exit [0x%x]\n", AgesaStatus);
  return  AgesaStatus;
}
