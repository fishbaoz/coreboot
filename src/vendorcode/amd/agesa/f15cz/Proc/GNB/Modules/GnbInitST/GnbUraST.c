...
#include  "Filecode.h"
#define FILECODE PROC_GNB_MODULES_GNBINITST_GNBURAST_FILECODE
...
/*----------------------------------------------------------------------------------------*/
/**
 * Gnb Unified Register Access READ method
 *
 *
  ...
  )
{
  ...
  case TYPE_GNB_PROTOCOL_ACCESS:
    ...
    GnbRegisterReadST (Device->GnbHandle, (UINT8)TargetAddress, UraTokenInfo->RegAddress, &RegValue, UraTokenInfo->Flags, Device->StdHeader);
...
/*----------------------------------------------------------------------------------------*/
/**
 * Gnb Unified Register Access WRITE method
 *
 *
 * @param[in]       Device           Pointer to Device object
 * @param[in]       UraTokenInfo     Pointer to URA_TOKEN_INFO structure
  ...
  )
{
  ...
  case TYPE_GNB_PROTOCOL_ACCESS:
    if (UraTokenInfo->WholeRegAccess == TRUE) {
      ...
      GnbRegisterReadST (Device->GnbHandle, UraTokenInfo->RegDomainType, UraTokenInfo->RegAddress, &RegValue, UraTokenInfo->Flags, Device->StdHeader);
      FieldMask = (((UINT32)1 << UraTokenInfo->BfWidth) - 1);
      TargetValue = RegValue & (~(FieldMask << UraTokenInfo->BfOffset));
      TargetValue |= (TempValue & FieldMask) << UraTokenInfo->BfOffset;
    }
    GnbRegisterWriteST (Device->GnbHandle, UraTokenInfo->RegDomainType, UraTokenInfo->RegAddress, &TargetValue, UraTokenInfo->Flags, Device->StdHeader);
...
/*----------------------------------------------------------------------------------------*/
/**
 * Gnb Unified Register Access WRITE MULTIPLE method
 *
 *
 * @param[in]       Device             Pointer to device object
 * @param[in]       UraTokenInfo       Pointer to URA_TOKEN_INFO structure
  ...
  )
{
  ...
  for (Index = 0; Index < CombinedCount; Index++) {
    ...
    case TYPE_GNB_PROTOCOL_ACCESS:
      TargetAddress = UraTokenInfo->RegDomainType;
      GnbRegisterWriteST (Device->GnbHandle, (UINT8)TargetAddress, StreamSetAddress, &RegValue, UraTokenInfo->Flags, Device->StdHeader);
      break;
