 ...
 * @e \$Revision: 314320 $   @e \$Date: 2015-03-09 14:16:16 +0800 (Mon, 09 Mar 2015) $
 *
 */
/*****************************************************************************
 *
  ...
  )
{
  ...
  IDS_PERF_TIMESTAMP (TP_BEGINAMDCPUMID, &MidParams->StdHeader);
  CalledStatus = AmdCpuMid (&MidParams->StdHeader, &MidParams->PlatformConfig);
  if (CalledStatus > AgesaStatus) {
    AgesaStatus = CalledStatus;
  }
  IDS_PERF_TIMESTAMP (TP_ENDAMDCPUMID, &MidParams->StdHeader);

  IDS_PERF_TIMESTAMP (TP_BEGINAMDGNBMIDLATE, &MidParams->StdHeader);
  CalledStatus = GnbInitAtMidLate (MidParams);
  if (CalledStatus > AgesaStatus) {
    AgesaStatus = CalledStatus;
  }
  IDS_PERF_TIMESTAMP (TP_ENDAMDAMDGNBMIDLATE, &MidParams->StdHeader);

  CalledStatus = AmdCpuMidLate (&MidParams->StdHeader, &MidParams->PlatformConfig);
  if (CalledStatus > AgesaStatus) {
    AgesaStatus = CalledStatus;
  }
