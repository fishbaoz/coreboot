  ...
  )
{
  ...
  if (ResumeParams->S3DataBlock.NvStorage != NULL) {
    ...
    if (AmdMemDoResume (&ResumeParams->StdHeader) == AGESA_SUCCESS) {
      ReturnStatus = AmdMemS3DatablobRevMatch (ResumeParams->S3DataBlock.NvStorage, &ResumeParams->StdHeader);
    ...
    if (ReturnStatus == AGESA_SUCCESS) {
      if (AmdMemDoResume (&ResumeParams->StdHeader) == AGESA_SUCCESS) {
        ...
        ApMsrSync[3].RegisterAddress = 0xC00110A2;
        ApMsrSync[4].RegisterAddress = 0xC0010030;
        ApMsrSync[5].RegisterAddress = 0xC0010031;
        ApMsrSync[6].RegisterAddress = 0xC0010032;
        ApMsrSync[7].RegisterAddress = 0xC0010033;
        ApMsrSync[8].RegisterAddress = 0xC0010034;
        ApMsrSync[9].RegisterAddress = 0xC0010035;
        ApMsrSync[10].RegisterAddress = 0;
      ...
      // If non-secure processor, ucode patch must be loaded by BIOS
      if (!CheckPspPlatformSecureEnable ()) {
        LoadMicrocodePatchInCurrentSocket (&ResumeParams->StdHeader);
      }
      if (ReturnStatus > AmdInitResumeStatus) {
        AmdInitResumeStatus = ReturnStatus;
      }
  ...
  )
{
  ...
  S3DataBlockInitializer (&ResumeParams->S3DataBlock);
