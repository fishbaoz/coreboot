...
extern u8 bus_avalon[3];

extern u32 bus_type[256];
extern u32 sbdn_avalon;
extern u32 apicid_avalon;
...
static void *smp_write_config_table(void *v)
{
	...
	apicid_avalon = IO_APIC_ID;
	smp_write_ioapic(mc, apicid_avalon, 0x21, dword);

	smp_write_ioapic(mc, apicid_avalon+1, 0x21, 0xFEC20000);
	/* PIC IRQ routine */
...
#define IO_LOCAL_INT(type, intr, apicid, pin)				\
	...
	mptable_add_isa_interrupts(mc, bus_isa, apicid_avalon, 0);

	/* PCI interrupts are level triggered, and are
	 * associated with a specific bus/device/function tuple.
	 */
#define PCI_INT(bus, dev, int_sign, pin)				\
        smp_write_intsrc(mc, mp_INT, MP_IRQ_TRIGGER_LEVEL|MP_IRQ_POLARITY_LOW, (bus), (((dev)<<2)|(int_sign)), apicid_avalon, (pin))
	...
	/* PCI slots */
	/* PCI_SLOT 0. */
	PCI_INT(bus_avalon[1], 0x5, 0x0, 0x14);
	PCI_INT(bus_avalon[1], 0x5, 0x1, 0x15);
	PCI_INT(bus_avalon[1], 0x5, 0x2, 0x16);
	PCI_INT(bus_avalon[1], 0x5, 0x3, 0x17);

	/* PCI_SLOT 1. */
	PCI_INT(bus_avalon[1], 0x6, 0x0, 0x15);
	PCI_INT(bus_avalon[1], 0x6, 0x1, 0x16);
	PCI_INT(bus_avalon[1], 0x6, 0x2, 0x17);
	PCI_INT(bus_avalon[1], 0x6, 0x3, 0x14);

	/* PCI_SLOT 2. */
	PCI_INT(bus_avalon[1], 0x7, 0x0, 0x16);
	PCI_INT(bus_avalon[1], 0x7, 0x1, 0x17);
	PCI_INT(bus_avalon[1], 0x7, 0x2, 0x14);
	PCI_INT(bus_avalon[1], 0x7, 0x3, 0x15);

	PCI_INT(bus_avalon[2], 0x0, 0x0, 0x12);
	PCI_INT(bus_avalon[2], 0x0, 0x1, 0x13);
	PCI_INT(bus_avalon[2], 0x0, 0x2, 0x14);
