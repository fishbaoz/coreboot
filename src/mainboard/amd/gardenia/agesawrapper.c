...
AGESA_STATUS agesawrapper_amdS3Save(void)
{
	...
	AmdInitRtbParamsPtr->StdHeader = AmdInterfaceParams.StdHeader;

	Status = AmdInitRtb(AmdInitRtbParamsPtr);
	if (Status != AGESA_SUCCESS) {
		agesawrapper_amdreadeventlog(AmdInterfaceParams.StdHeader.HeapStatus);
		ASSERT(Status == AGESA_SUCCESS);
	}
	...
	printk(BIOS_DEBUG, "NvStorageSize=%x, NvStorage=%x\n",
	       (unsigned int)AmdInitRtbParamsPtr->S3DataBlock.NvStorageSize,
	       (unsigned int)AmdInitRtbParamsPtr->S3DataBlock.NvStorage);
#if 0
...
//	PspMboxBiosCmdS3Info (AmdS3SaveParamsPtr->S3DataBlock.NvStorage, AmdS3SaveParamsPtr->S3DataBlock.NvStorageSize);
	...
	printk(BIOS_DEBUG, "VolatileStorageSize=%x, VolatileStorage=%x\n",
	       (unsigned int)AmdInitRtbParamsPtr->S3DataBlock.VolatileStorageSize,
	       (unsigned int)AmdInitRtbParamsPtr->S3DataBlock.VolatileStorage);
