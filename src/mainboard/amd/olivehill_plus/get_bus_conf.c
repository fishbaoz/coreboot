...
void get_bus_conf(void)
{
	...
	memset(bus_avalon, 0, sizeof(bus_avalon));
	...
	//  bus_avalon[0] = (sysconf.pci1234[0] >> 16) & 0xff;
	bus_avalon[0] = (pci1234x[0] >> 16) & 0xff;

	/* avalon */
	dev = dev_find_slot(bus_avalon[0], PCI_DEVFN(sbdn_avalon + 0x14, 4));

	if (dev) {
		bus_avalon[1] = pci_read_config8(dev, PCI_SECONDARY_BUS);

		bus_isa = pci_read_config8(dev, PCI_SUBORDINATE_BUS);
		bus_isa++;
		for (j = bus_avalon[1]; j < bus_isa; j++)
			bus_type[j] = 1;
	}

	for (i = 0; i < 4; i++) {
		dev = dev_find_slot(bus_avalon[0], PCI_DEVFN(sbdn_avalon + 0x14, i));
		if (dev) {
			bus_avalon[2 + i] = pci_read_config8(dev, PCI_SECONDARY_BUS);
			bus_isa = pci_read_config8(dev, PCI_SUBORDINATE_BUS);
			bus_isa++;
		}
	}
	for (j = bus_avalon[2]; j < bus_isa; j++)
		bus_type[j] = 1;

	/* I/O APICs:   APIC ID Version State   Address */
	bus_isa = 10;
	apicid_base = CONFIG_MAX_CPUS;
	apicid_avalon = apicid_base;
}
